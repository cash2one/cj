...
class ip2address {
	...
	protected function _get_by_taobao() {
		...
		if (!$this->_get_json_by_httprequest($sdata, $url)) {
			...
			logger::error('taobao ip api error: '.$url.'|'.$this->errcode.'|'.$this->errmsg);
			return false;
		}
		...
		if (!empty($sdata['code'])) {
			...
			logger::error('tobao ip api error:'.$url.'|'.$this->errcode.'|'.$this->errmsg.'|'.$sdata['code'].'|'.$sdata['data']);
			return false;
		}
	...
	/**
	 * 通过http请求获取json结果数据
	 * @param string $data <strong style="color:red">(引用结果)</strong>返回的结果
	 * @param string $url 请求的url
	 * @param string $post 请求的数据
	 * @param array $http_header 请求的header头信息
	 * @param string $http_method 请求方法
	 * @param object $snoopy_reporting <strong style="color:red">(引用结果)</strong>snoopy信息
	 * @return boolean
	 */
	protected function _get_json_by_httprequest(&$data, $url, $post = array(), $http_header = array(), $http_method = 'GET', &$snoopy_reporting = array()) {
		...
		if ($data === null) {
			...
			logger::error('json parse error: '.$this->__request_url.'|'.$http_data.'|'.$this->errcode.'|'.$this->errmsg);
			return false;
		}
	...
	/**
	 * http请求私有方法
	 * @param string $data <strong style="color:red">(引用结果)</strong>请求结果
	 * @param string $url 请求的url
	 * @param string $post 请求的数据
	 * @param array $http_header 请求的header头信息
	 * @param string $http_method 请求方法
	...
	protected function __httprequest(&$data, $url, $post = array(), $http_header = array(), $http_method = 'GET', &$snoopy_reporting = array()) {
		...
		if (!$result || 200 != $this->__snoopy->status) {
			...
			logger::error('$snoopy->submit error: '.$url.'|'.$result.'|'.$this->__snoopy->status.'|'.$this->errcode.'|'.$this->errmsg);
			return false;
		}
