...
class upload {
	...
	private $_media_type_maps = array(
		// 音频、声音文件
		voa_d_oa_common_attachment::MEDIA_TYPE_VOICE => array('mp3'),
		// 视频文件
		voa_d_oa_common_attachment::MEDIA_TYPE_VIDEO => array('mp4')
	);
	...
	/**
	 * 构造函数，处理上传业务
	 * @param string $file_field 上传文件表单控件名称 或 base64编码字符串表单控件名 或 远程图片url 或 直接传入$_FILES[key]的数组（此方式用于处理多个文件上传）
	 * @param array $config 配置项，详见成员初始化介绍
	 * <pre>
	 ...
	 * </pre>
	 * @param bool $type 上传类型 remote|base64|upload|local，默认：upload。
	 * <pre>
	...
	*/
	...
	public function get_file_info() {
		...
		if ($this->_is_image) {
			// 图片文件
			$media_type = voa_d_oa_common_attachment::MEDIA_TYPE_IMAGE;
		} else {
			// 先初始化为普通文件
			$media_type = voa_d_oa_common_attachment::MEDIA_TYPE_FILE;
			// 遍历以判断具体的文件类型
			foreach ($this->_media_type_maps as $_type => $_exts) {
				if (in_array($this->_file_type, $_exts)) {
					$media_type = $_type;
					break;
				}
			}
		...
		return array(
				...
				'media_type' => $media_type,// 文件类型，输出常量值
				...
				'config' => $this->_config,// 经过验证后的配置信息
		);
	}
...
/*		if (!in_array($file_type, $this->_config['allow_files'])) {
	...
	private function _get_image_ext($filename) {
		...
		if ($get_image_size && is_array($get_image_size) && !empty($get_image_size[2])) {
			...
			switch ($get_image_size[2]) {
				case self::GIF: return 'gif';
				case self::JPG: return 'jpg';
				case self::PNG: return 'png';
				case self::SWF: return 'swf';
				case self::PSD: return 'psd';
				case self::BMP: return 'bmp';
				case self::TIFF: return 'tiff';
				case self::TIFF2: return 'tiff';
				case self::JPC: return 'jpc';
				case self::JP2: return 'jp2';
				case self::JPX: return 'jpx';
				case self::JB2: return 'jb2';
				case self::SWC: return 'swc';
				case self::IFF: return 'iff';
				case self::WBMP: return 'wbmp';
				case self::XBM: return 'xbm';
			}
		}
	...
	private function _create_tmpfile($data) {
		...
		$temp_file = tempnam(sys_get_temp_dir(), md5(startup_env::get('timestamp')) . sprintf('%04s', mt_rand(1,9999)));
