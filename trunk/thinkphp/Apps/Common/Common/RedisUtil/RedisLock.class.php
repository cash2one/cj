 ...
 * redis 分布式锁实现
 */
namespace Common\Common\RedisUtil;
class RedisLock extends RedisClient{
    ...
    public function __construct($config_redis) {
        $this->redisString = parent::getInstance($config_redis);
    }
    ...
    /**
     ...
     public function unlock($name) {
          ...
          if ($this->isLocking($name)) {
                 //删除锁
              if ($this->redisString->delKey("lock_$name")) {
                 //清掉lockedNames里的锁标志
     ...
     public function expire($name, $expire) {
         ...
         if ($this->isLocking($name)) {
             ...
             $expire = max($expire, 1);
             //增加锁生存时间
             if ($this->redisString->expire("lock_$name", $expire)) {
                 return true;
             }
         }
         return false;
     ...
     public function isLocking($name) {
         ...
         if (isset($this->lockedNames[$name])) {
                     //从redis返回该锁的生存时间
             return $this->lockedNames[$name] = $this->redisString->get("lock_$name");
         }
    ...
    public function set($key, $value, $exp=0){
        return $this->redisString->set($key, $value, $exp);
    }
    ...
    public function ttl($key){
        return $this->redisString->ttl($key);
    }
