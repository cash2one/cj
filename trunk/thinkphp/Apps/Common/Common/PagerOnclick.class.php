...
/**
 * PagerOnclick.php
 * 分页插件 禁用href 使用pageOnclick(page)
 * @author: anything
 * @createTime: 2015/11/23 10:18
 * @version: $Id$ 
 * @copyright: 畅移信息
 *
 *
...
class PagerOnclick {
    ...
    public function get_page_url($page_id, $page_name = null, $is_return_link = false) {
        ...
        if ($page_name === null) {
            if ($this->_current_page == $page_id) {
                ...
                $page_url = sprintf(
                    '<li%s><a href="javascript:void(0);" title="%d" onclick="pageOnclick(%d)">%d</a></li>',
                    $this->_link_class_string,  $page_id, $page_id, $page_id
                );
            }
        } else {
            if ($this->_current_page == $page_id) {
                ...
                if ($page_name == $this->_alt_prev) {
                    $page_url = sprintf(
                        '<li><a%s href="javascript:void(0);" title="%s" onclick="pageOnclick(%d)">%s</a></li>',
                        $this->_alt_prev_class_string, $page_name, $page_id, $this->_alt_prev_icon
                    );
                } else if ($page_name == $this->_alt_next) {
                    $page_url = sprintf(
                        '<li><a%s href="javascript:void(0);" title="%s" onclick="pageOnclick(%d)">%s</a></li>',
                        $this->_alt_next_class_string,  $page_name, $page_id, $this->_alt_next_icon
                    );
                } else {
                    $page_url = sprintf(
                        '<li%s><a href="javascript:void(0);" title="%d" onclick="pageOnclick(%d)">%d</a></li>',
    ...
    protected function _get_page_link() {
        ...
        $get_params = Request::get_instance()->getx();
    ...
    public static function resolve_options(&$options) {
        ...
        if (empty($options['current_page'])) {
            $options['current_page'] = Request::get_instance()->get('page', 1);
        }
