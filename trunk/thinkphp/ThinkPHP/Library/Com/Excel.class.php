...
class Excel extends \PHPExcel {
	...
	private function __set_attributes($attrs = array()) {
		...
		$this->getProperties()->setCreator($attrs['creator'])->setLastModifiedBy($attrs['last_modified'])->setTitle($attrs['title'])->setSubject($attrs['subject'])->setDescription($attrs['description'])->setKeywords($attrs['keywords'])->setCategory($attrs['category']);
	}
	...
	private function __set_options($options = array()) {
		...
		$allowed_options = array('title_text_color', 'title_background_color');
	...
	private function __set_row_title($titles) {
		...
		if ('assoc' == $this->__array_type($titles)) {
			foreach ($titles as $_column => $_val) {
				...
				$this->getActiveSheet()->getStyle($_column . '1')->getFont()->setBold(true);
				$this->getActiveSheet()->getStyle($_column . '1')->getFont()->getColor()->setARGB($this->__title_text_color);
				$this->getActiveSheet()->getStyle($_column . '1')->getFill()->setFillType(\PHPExcel_Style_Fill::FILL_SOLID);
				$this->getActiveSheet()->getStyle($_column . '1')->getFill()->getStartColor()->setARGB($this->__title_background_color);
				$this->getActiveSheet()->getStyle($_column)->getNumberFormat()->setFormatCode(\PHPExcel_Style_NumberFormat::FORMAT_TEXT);
			}
		} else {
			$start = 'A';
	 ...
	 * 设置行内容
	 *
	 * @param array $xls_rows e.g. $xls_rows = array(
		...
		foreach ($xls_rows as $_row) {
			if ('assoc' == $this->__array_type($_row)) { // 关联
				foreach ($_row as $_column => $_val) {
					...
					$this->getActiveSheet()->getStyle($_column . $n)->getAlignment()->setWrapText(true)->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER);
				}
			} else {
				$start = 'A';
	...
	private function __set_row_width($widths = array()) {
		...
		if ('assoc' == $this->__array_type($widths)) {
			...
			foreach ($widths as $_column => $_val) {
				if ('auto' == $_val || 0 == $_val) {
					$this->getActiveSheet()->getColumnDimension($_column)->setAutoSize(true);
				} else {
					$this->getActiveSheet()->getColumnDimension($_column)->setWidth($_val . "pt");
				}
	 ...
	 * 下载生成的Excel文件
	 *
	 * @param string $download_file 下载的文件名
	 */
