...
class Dnspod {
	...
	public function __construct() {
		...
		if (! function_exists('curl_init')) {
			E('API need the cURL PHP extension.');
		}

		if (! function_exists('json_decode')) {
			E('API need the JSON PHP extension.');
		}

		if (! function_exists('iconv')) {
			E('API need the iconv PHP extension.');
		}

		// 读取配置
		$this->__user = cfg('DNSPOD.USER');
		$this->__passwd = cfg('DNSPOD.PASSWD');
		$this->__zoneid = cfg('DNSPOD.ZONEID');
	}
	...
	protected function _call_api($api, $params, &$result) {
		...
		\Think\Log::record($html);
		if ($result['status']['code'] != 1) {
			E($result['status']['message']);
			return false;
		}
