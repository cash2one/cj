...
class Model {
	...
	public function fetch_array($sql, $params = array(), $options = null) {
		...
		if (! empty($options)) {
			...
			if (is_array($options)) {
				...
				if (0 >= $start && 0 >= $limit) {
					// do nothing
				} else if (0 >= $start || 0 >= $limit) { // 如果只有一个值
	...
	private function __prepare(&$sql, $params) {
		...
		// 处理字段
		$sql = preg_replace_callback("/(([a-z0-9_\.]+)(\s*)(=|>|<|!))/i", array($this, '_cb_pa'), $sql);
		$sql = preg_replace_callback("/(([a-z0-9_\.]+)(\s+)(like|not like|in|not in|between|not between))(\s+)/i", array($this, '_cb_pb'), $sql);
		// 匹配占位符以及参数
