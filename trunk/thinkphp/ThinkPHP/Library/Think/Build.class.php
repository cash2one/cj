...
';
	...
	static public function buildController($module, $controller = '') {
		...
		if (! is_file($file)) {
			$content = str_replace(
				array('[MODULE]', '[CONTROLLER]', '[CNAME_CPATH]', '[CNAME]', '[DEFAULT_ACTION]', '[TPL_PATH]', '[BASE_CONTROLLER]'),
				array($module, $act, $cname_cpath, $c_layer, $acname, $tpl_path, $base_controller),
				$controller_tpl
			);
		...
		/**$c_layer = ucfirst(C('DEFAULT_C_LAYER'));
		$default_act = C('DEFAULT_CONTROLLER');
		$file = APP_PATH . $module . '/' . $c_layer . '/' . $default_act . $c_layer . EXT;
		if (! is_file($file)) {
			$content = str_replace(
				array('[MODULE]', '[CONTROLLER]', '[CNAME_CPATH]', '[CNAME]', '[DEFAULT_ACTION]'),
				array($module, $controller, $c_layer.'\\'.str_replace('/', '\\', $default_act), $c_layer, C('DEFAULT_ACTION')),
				self::$controller
			);
			if (! C('APP_USE_NAMESPACE')) {
				$content = preg_replace('/namespace\s(.*?);/', '', $content, 1);
	...
	static public function buildModel($module, $model) {
		...
		$content = str_replace(array('[MODULE]', '[MODEL]'), array($module, $model), $tpl);
		// 如果启用了命名空间
		if (! C('APP_USE_NAMESPACE')) {
			$content = preg_replace('/namespace\s(.*?);/', '', $content, 1);
		}
	...
	static public function buildAction($module, $controller, $action, $isapi = false) {
		...
		// 解析操作方法
		$action = str_replace(array('[ACTION_NAME]', '[TPL_PATH]'), array($actionName, $tpl_path), $isapi ? self::$action_api : self::$action);
		// 读取类文件
