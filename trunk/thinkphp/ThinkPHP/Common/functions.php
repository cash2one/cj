...
function S($name, $value = '', $options = null) {
	...
	if ('' === $value) { // 获取缓存
		...
		// 记录缓存
		if (function_exists('syscache_record')) {
...
function url_uniqueid() {
	...
	$str = boardurl() . random(16) . NOW_TIME;
	$uniqueid = md5($str);
	return $uniqueid;
}
...
function rfopen(&$data, $url, $post = null, $headers = array(), $method = 'GET', $ret_snoopy = false, $json = true, $files = null) {
	...
	if (! $result || 200 != $snoopy->status) {
		\Think\Log::record('$snoopy['.$method.'] error: ' . $url . '|' . $post . '|' . $result . '|' . $snoopy->status);
	...
	// 如果返回的数据为空, 则
	if (empty($data)) {
		\Think\Log::record('$snoopy['.$method.'] error: ' . $url . '|' . $result . '|' . $snoopy->status);
		$data = $snoopy; // 出错时, 返回 $snoopy 对象
...
function extract_field(&$out, $fields, $in = array()) {
	...
	foreach ($fields as $_in_k => $_rule) {
		...
		// 类型强制转换
		switch ($type) {
			case 'array': $val = empty($val) ? array() : (array)$val; break;
 ...
 * 拼凑 api 数据返回值结构
 * @param array $data 返回数据
 * @return multitype:string unknown NULL mixed
 */
