...
class voa_backend_cron_inspect extends voa_backend_base {
	/** 外部传入的参数 */
	...
	private function __send(&$count) {
		/** 读取队列 */
		$tasks = &service::factory('voa_s_oa_inspect_tasks', array('pluginid' => 0));
		// 读取执行中的， 本时间段需要提醒的。
		$condi = array('it_execution_status'=>'2',
				'it_alert_time'=>rgmdate(startup_env::get('timestamp')+120, 'H:00'),
				'it_end_date'=>array(startup_env::get('timestamp'), '>'),
		);

		$data = $tasks->fetch_by_conditions($condi, $count, $this->__perpage);
		foreach ($data as $item) {
			if ($item['it_repeat_frequency'] != 'no') {
				$current_date = rgmdate(startup_env::get('timestamp'), 'Y-m-d');
				$last_execution_date = rgmdate($item['it_last_execution_time'], 'Y-m-d');
				// 上次执行日期不是当前日期则下一步
				if ($last_execution_date != $current_date) {
					$repeat_type = explode('_', $item['it_repeat_frequency']);
					// 重复执行计划的类型
					if ($repeat_type[0] == 'day') {// 每天
						if ($repeat_type[1] == 1) {
							$this->_execution($item);
						} elseif ($repeat_type[1] > 1){
							$next_execution_date = rgmdate($item['it_last_execution_time'] + 86400 * $repeat_type[1], 'Y-m-d');
							//下一次执行日期等于当前日期则执行
	...
	private function _execution($item) {
		$item['it_parent_id'] = $item['it_id'];
		$item['it_last_execution_time'] = $item['it_last_execution_time'];
		$item['it_repeat_frequency'] = 'no';
		unset($item['it_id']);
		unset($item['it_alert_time']);
		$id = $this->_service_single('inspect_tasks', $this->_module_plugin_id, 'insert', $item, true);
		$item['it_id'] = $id;
		$ins_insert = &uda::factory('voa_uda_frontend_inspect_insert');
