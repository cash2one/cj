 ...
 * tjsite.php
 * 统计指定站点的数据
 * @uses php tool.php -n tjsite -dbname ep_10002
 * Create By Deepseath
 * $Author$
 * $Id$
 */

class voa_backend_tool_tjsite extends voa_backend_base {
	/** 参数 */
	...
	public function main() {
		...
		while ($row = $this->_db->fetch_array($q)) {
			...
			if (rgmdate($row['ep_created'], 'Y-m') != '2015-06') {
				//continue;
			}
		...
		$output .= "\r\n";
		$output .= implode("\r\n", $put_data_list);
		file_put_contents(APP_PATH.'/data/tjsite_'.rgmdate(startup_env::get('timestamp'), 'Ymd').'.csv', $output);
	...
	protected function _site(&$result) {
		...
		while ($row = $this->_db->fetch_array($q)) {
			...
			$result[$row['cp_identifier']] = array(
				$count,
				$count_do
			);

			$count_total = $count_total + $count + $count_do;
		}
	...
	protected function _ct_sign(&$count, &$count_do) {
		...
		$q = $this->_db->query("SELECT COUNT(*) FROM oa_sign_detail WHERE sd_created>{$this->_ts}");
		$count = $this->_db->result($q) + $count;
	...
	protected function _ct_nvote(&$count = 0, &$count_do = 0) {
		...
		// 投票次数
		$count_do = (int)$this->_db->result_first("SELECT COUNT(`id`) FROM `oa_nvote_mem` WHERE `updated`>{$this->_ts}");
		$count_do += $count;
	...
	protected function _ct_activity(&$count = 0, &$count_do = 0) {
		...
		// 报名次数
		$count_do = (int)$this->_db->result_first("SELECT COUNT(`apid`) FROM `oa_activity_partake` WHERE `updated`>{$this->_ts}");
	...
	protected function _ct_express(&$count = 0, &$count_do = 0) {
		...
		// 快递响应次数
		$count_do = (int)$this->_db->result_first("SELECT COUNT(`mid`) FROM `oa_express_mem` WHERE `updated`>{$this->_ts}");
	...
	protected function _ct_blessingredpack(&$count = 0, &$count_do = 0) {
		...
		$count_do = 0;
		// 红包发起次数
		$count = (int)$this->_db->result_first("SELECT COUNT(*) FROM `oa_blessing_redpack` WHERE `created`>{$this->_ts}");
		// 红包领取次数
		$count_do = (int)$this->_db->result_first("SELECT COUNT(*) FROM `oa_blessing_redpack_log` WHERE `created`>{$this->_ts}");
	...
	// 培训
	protected function _cb_jobtrain(&$count = 0, &$count_do = 0) {

		$count = 0;
		$count_do = 0;
		// 发起数
		$count = (int)$this->_db->result_first("SELECT COUNT(*) FROM `oa_jobtrain_article` WHERE `created`>{$this->_ts}");
		// 回复数
		$count_do = (int)$this->_db->result_first("SELECT COUNT(*) FROM `oa_jobtrain_coll` WHERE `created`>{$this->_ts}");
		$count_do = $count_do + (int)$this->_db->result_first("SELECT COUNT(*) FROM `oa_jobtrain_comment` WHERE `created`>{$this->_ts}");
		$count_do = $count_do + (int)$this->_db->result_first("SELECT COUNT(*) FROM `oa_jobtrain_comment_zan` WHERE `created`>{$this->_ts}");
	...
	// 考试
	protected function _cb_exam(&$count = 0, &$count_do = 0) {

		$count = 0;
		$count_do = 0;

		$count = (int)$this->_db->result_first("SELECT COUNT(*) FROM `oa_exam_paper` WHERE `created`>{$this->_ts}");
		$count_do = (int)$this->_db->result_first("SELECT COUNT(*) FROM `oa_exam_ti_tj` WHERE `create`>{$this->_ts}");
