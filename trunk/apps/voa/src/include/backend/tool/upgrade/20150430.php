...
/**
 * 销售管理上线脚本
 * php -q tool.php -n upgrade -version 20150430 -epid vchangyi_oa_upgrade
 * User: tim_zhang
 * Date: 15/4/30
 * Time: 上午10:27
 */
class execute {
    ...
    protected function _open_plugin() {
        ...
        $timestamp = time();
        // 先尝试删除
        $this->_db->query("DELETE FROM `oa_sale_plugin` WHERE `cp_identifier`='{$this->__identifier}'");
        // 新增
        $this->_db->query("INSERT INTO `oa_sale_plugin` (`cp_identifier`,
				...
				'{$this->__identifier}', 1, {$cpg_id}, '', '', 1030, 0, 0, '{$plugin_name}', '{$this->__identifier}.png',
