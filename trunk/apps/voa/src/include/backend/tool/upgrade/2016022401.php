...
class execute {
	...
	public function run() {
        ...
        if ($this->_db->fetch_row($query)) {
            //插入sign_setting out_sign_include 外出考勤记为出勤开关标志
            ...
            if (!$this->_db->fetch_row($query2)) {
                $this->_db->query("
                  INSERT INTO oa_common_cpmenu
                   ...
                   VALUES ('14', '0', 'office', 'sign', 'config', 'subop', '0', '设置', 'fa-gear', '106', '999', '1', '1', '0', '0', '0');
    ...
    /**
     * 入sign_setting out_sign_include 外出考勤记为出勤开关标志
     */
    protected function _insert_sign_setting(){
        //判断该菜单是否存在
        $query = $this->_db->query("SELECT * FROM oa_sign_setting WHERE ss_key = 'out_sign_include' ");
        ...
        if (!$this->_db->fetch_row($query)) {
            $this->_db->query("
                  ...
                  VALUES
                  ('out_sign_include', '1', '0', '外出考勤计为出勤开关 1-关闭;2-开起', '1', '1456139786', '1456292144', '0');
            ");
        }
    }
