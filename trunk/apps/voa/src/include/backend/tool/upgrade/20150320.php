 ...
 * 20150320.php
 * 新闻公告上线
 * php -q tool.php -n upgrade -version 20150320 -epid vchangyi_oa_upgrade
 * Create By Deepseath
 * $Author$
 * $Id$
 */
class execute {
	 ...
	 * 新增套件
	 */
	protected function _add_suite() {

		// 新套件信息
		$suite = array(
			'cpg_id' => 5,
			'cpg_suiteid' => 'tj407a156836450616',
			'cpg_name' => '企业文化',
			'cpg_icon' => 'fa-group',
			'cpg_ordernum' => 0,
			'cpg_status' => 1,
		...
		$this->_db->query("INSERT INTO `oa_common_plugin_group`
				(`".implode("`, `", array_keys($suite))."`) VALUES
				('".implode("', '", array_values($suite))."')");
	...
	protected function _open_plugin() {
		...
		$this->_db->query("INSERT INTO `oa_common_plugin` (`cp_pluginid`,`cp_identifier`,
				...
				$cp_pluginid,
				'{$this->__identifier}', 1, {$cpg_id}, '', '', 10{$cp_pluginid}, 0, 0, '{$plugin_name}', '{$this->__identifier}.png',
