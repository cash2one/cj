...
class execute {
	...
	protected function _plugin_table() {
		$this->_db->query("ALTER TABLE `oa_askfor_template`  ADD `positions` TEXT NOT NULL COMMENT '职务json'  AFTER `approvers`;");
		$this->_db->query("ALTER TABLE `oa_askfor_proc`  ADD `mp_id` INT(10) UNSIGNED NOT NULL COMMENT '职务ID'  AFTER `m_username`;");
		$this->_db->query("ALTER TABLE `oa_askfor_proc`  ADD `mp_name` VARCHAR(100) NOT NULL COMMENT '职务名称'  AFTER `m_username`;");
		return true;
	}
