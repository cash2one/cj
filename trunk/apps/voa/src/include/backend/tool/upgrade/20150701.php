...
class execute {
	...
	protected function _plugin_table() {
		...
		$this->_db->query("INSERT INTO vcycenter.uc_enterprise_adminer(
			ep_id, ca_id, mobilephone, status, created, updated, deleted
			) SELECT (SELECT cs_value FROM oa_common_setting WHERE cs_key = 'ep_id' LIMIT 1) ep_id,
			ca_id, ca_mobilephone mobilephone, ca_status status, ca_created created, ca_updated updated,
			ca_deleted deleted FROM oa_common_adminer;");

		// 判断应用表是否存在
		$query = $this->_db->query("SHOW TABLES LIKE 'oa_news'");
		if ($this->_db->fetch_row($query)) {
			$this->_db->query("ALTER TABLE `oa_news`
				ADD `is_like` tinyint(3) unsigned NOT NULL DEFAULT '0' COMMENT '是否开启赞：0=不开启；1=开启' AFTER `check_summary`,
				ADD `num_like` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '点赞次数' AFTER `is_like`, COMMENT='新闻公告表';");

			$this->_db->query("CREATE TABLE IF NOT EXISTS `oa_news_like` (
	 			...
	 			`m_uid` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '点赞用户uid',
	  			...
	  			`ip` varchar(150) NOT NULL DEFAULT '' COMMENT 'ip地址',
			...
			$this->_db->query("DROP TABLE IF EXISTS`oa_news_like`;");
		}
