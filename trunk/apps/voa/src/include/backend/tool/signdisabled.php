 ...
 * signdisabled.php
 * 统计指定站点的数据
 * @uses php tool.php -n signdisabled -dbname ep_10002 -sdate 2016-01-01 00:00:00 -edate 2106-01-01 00:00:00
 * Create By Deepseath
 * $Author$
 * $Id$
 */

class voa_backend_tool_signdisabled extends voa_backend_base {
	/** 参数 */
	...
	public function main() {
		...
		file_put_contents(APP_PATH.'/data/signdisabled_'.$ymd.'.csv', $output);
	...
	protected function _site(&$result) {
		...
		//判断是否安装考勤，并且考勤记录上上周有数据，上周没数据
		$q = $this->_db->query('SELECT * FROM oa_common_plugin WHERE cp_available>0 AND cp_available<255 AND cp_identifier = "sign"');
		...
		$time_lastweek_begin = rstrtotime('2016-04-04 00:00:00');
		$time_lastweek_end = rstrtotime('2016-04-11 00:00:00');
		$time_current_begin = rstrtotime('2016-04-11 00:00:00');
		$time_current_end = rstrtotime('2016-04-18 00:00:00');

		// 上上周有记录
		$q = $this->_db->query("SELECT COUNT(*) FROM oa_sign_record WHERE sr_created>{$time_lastweek_begin} AND sr_created<{$time_lastweek_end} AND sr_status < 3");
		$count_lastweek = $this->_db->result($q);

		$q = $this->_db->query("SELECT COUNT(*) FROM oa_sign_location WHERE sl_created>{$time_lastweek_begin} AND sl_created<{$time_lastweek_end} AND sl_status < 3");
		$count_lastweek = $this->_db->result($q) + $count_lastweek;

		//上周没记录
		$q = $this->_db->query("SELECT COUNT(*) FROM oa_sign_record WHERE sr_created>{$time_current_begin} AND sr_created<{$time_current_end} AND sr_status < 3");
		$count_current = $this->_db->result($q);

		$q = $this->_db->query("SELECT COUNT(*) FROM oa_sign_location WHERE sl_created>{$time_current_begin} AND sl_created<{$time_current_end} AND sl_status < 3");
		$count_current = $this->_db->result($q) + $count_current;


		if ($count_lastweek > 0 && $count_current == 0) {
			//继续统计
