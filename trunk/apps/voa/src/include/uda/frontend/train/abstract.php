<?php
/**
 * voa_uda_frontend_train_abstract
 * 统一数据访问/培训/基类
 *
 * $Author$
 * $Id$
...
class voa_uda_frontend_train_abstract extends voa_uda_frontend_base {

	public $d_class ;  //train目录下d层类
	/** 应用的唯一标识名 */
	...
	public function __construct($ptname = array()) {
		...
		foreach ($d_train_setting->list_all() as $s) {
			if ($d_train_setting::TYPE_ARRAY == $s['type']) {
