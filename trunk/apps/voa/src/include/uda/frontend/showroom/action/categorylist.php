<?php
/**
 * voa_uda_frontend_showroom_action_categorylist
 * 统一数据访问/陈列/目录列表
 *
 * $Author$
 * $Id$
...
class voa_uda_frontend_showroom_action_categorylist extends voa_uda_frontend_showroom_abstract {
	...
	protected function _list_categories_by_conds($conds, $page) {
		...
		$category = new voa_d_oa_showroom_category();
		$categoryright = new voa_d_oa_showroom_categoryright();
		$tc_ids = array();
	...
	public function list_categories_by_is_all() {
		...
		$result = array();
		$categoryright = new voa_d_oa_showroom_categoryright();
		$categories  = $categoryright->list_by_conds(array('is_all' => voa_d_oa_showroom_categoryright::IS_ALL));
		if ($categories) {
