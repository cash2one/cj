<?php
/**
 * voa_uda_frontend_showroom_action_articleview
 * 统一数据访问/陈列/文章查看
 *
 * $Author$
 * $Id$
...
class voa_uda_frontend_showroom_action_articleview extends voa_uda_frontend_showroom_abstract {
	...
	public function view( $id, $m_uid = 0 ) {
		...
		$d_article = new voa_d_oa_showroom_article();
	...
	protected  function _get_article_content($id) {
		...
		$d_article_content = new voa_d_oa_showroom_articlecontent();
	...
	protected function _get_rights($id, &$article) {
		...
		$d_article_right = new voa_d_oa_showroom_articleright();
		$article_rights = $d_article_right->list_by_conds(array('ta_id'=>$id));
		if ($article_rights) {
			foreach ($article_rights as $right) {
				$article['is_all'] = voa_d_oa_showroom_articleright::IS_ALL;
				if ($right['m_uid'] != 0 && $right['cd_id'] == 0) {//人员
					$article['contacts'][] = $right['m_uid'];
					$article['is_all'] = voa_d_oa_showroom_articleright::NOT_IS_ALL;
				}
