...
/**
 * voa_uda_frontend_cinvite_updata
 * 邀请人员/uda/更新数据
 * Created by zhoutao.
 ...
 */
class voa_uda_frontend_cinvite_updata extends voa_uda_frontend_cinvite_base {
	/**
	 * 方便扩展,保留之
	 */
	...
	public function invite_setting_data( $postx, &$out ) {
		...
		$out = array(
			...
			'short_title' => (string) $postx['short_title'], // 邀请标题
	...
	public function update_invite( $data, $out ) {
		...
		foreach( $data as $k => $v ) {
			if ($this->_serv_setting->get_by_conds(array('key' => $k))) {
				$this->_serv_setting->update_by_conds(array('key' => $k), array('value' => $v));
			} else {
				$this->_serv_setting->insert(array('key' => $k, 'value' => $v));
			}
		}
