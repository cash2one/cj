<?php
/**
 * voa_uda_frontend_cnvote_mem_option_add
 * 添加用户投票
 * User: luckwang
 * Date: 15/3/9
 * Time: 下午4:55
...
class voa_uda_frontend_cnvote_mem_option_add extends voa_uda_frontend_cnvote_mem_option_abstract {
    ...
    public function __construct()
    {
        ...
        $this->_serv_option = &service::factory('voa_s_oa_cnvote_option');
        $this->_serv_vote = &service::factory('voa_s_oa_cnvote');
        $this->_serv_mem = &service::factory('voa_s_oa_cnvote_mem');
    }
    ...
    public function vote($nvo_ids, $m_uid) {
        ...
        if ($is_can_vote == 2) {
            ...
            //修改投票
            if ($nvote['is_repeat'] == voa_d_oa_cnvote::REPEAT_YES) {
        ...
        //验证用户是否已经投过票
        if (!empty($mem) && $mem['is_nvote'] == voa_d_oa_cnvote_mem::NVOTE_YES) {
        ...
        try {
            ...
            $this->_serv_mem->update_by_conds(array(
                                                'nvote_id =?' => $nvote_id,
                                                'm_uid =?' => $m_uid,
                                                'is_nvote =?' => voa_d_oa_cnvote_mem::NVOTE_NO
                                                ),
                                                array('is_nvote' => voa_d_oa_cnvote_mem::NVOTE_YES));
