<?php
/**
 * voa_uda_frontend_express_mem_list
 * 统一数据访问/社区应用/获取评论列表
 *
 * $Author$
 * $Id$
...
class voa_uda_frontend_express_mem_list extends voa_uda_frontend_express_mem_abstract {
	...
	public function execute($in, &$out) {
		...
		// 查询表格的条件
		$fields = array(
			array('eid', self::VAR_ARR, null, null, true),
	 ...
	 * @return boolean
	 */
	public function get_by_conds($in, &$out) {
		...
		// 读取总数
		$this->_total = $this->_serv->count_by_conds_left_join($conds);
		
		// 读取
		$out = $this->_serv->list_mem_join_express($conds, $option);
		if (empty($out)) {
			$out = array();
		}
