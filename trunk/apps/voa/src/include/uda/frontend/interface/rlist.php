...
class voa_uda_frontend_interface_rlist extends voa_uda_frontend_interface_abstract {
	...
	public function execute($in, &$out) {
		...
		$this->_total = $t->list_by_conds_join_count($conds);

		// 读取表格字段
		$out = $t->list_by_conds_join($conds, $option, array('log.created' => 'DESC'));
		if (empty($out)) {
			$out = array();
		}
