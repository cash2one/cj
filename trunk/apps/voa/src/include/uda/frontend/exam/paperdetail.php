<?php
/**
 * voa_uda_frontend_exam_paperdetail
 * 统一数据访问/试卷相关试题操作
 * Create By wogu
 * $Author$
 * $Id$
...
class voa_uda_frontend_exam_paperdetail extends voa_uda_frontend_base {
	...
	public function add_detail(array $request, $args) {
		...
		try {
			...
			$paper = $s_paper->get_by_id($args['id']);
			$s_paper->update_by_conds(array('id' => $args['id']), array('total_score' => $total_score, 'ti_num' => count($tis), 'status'=>$paper['status'] ));
	...
	public function edit_detail(array $request, $args) {
		...
		try {
			...
			if(!empty($deleteids)) {
				...
				$s_paper = new voa_s_oa_exam_paper();
				$paper = $s_paper->get_by_id($args['id']);
				$s_paper->update_by_conds(array('id' => $args['id']), array('total_score' => $total_score, 'ti_num' => $ti_num, 'status'=>$paper['status']));
			}
		} catch (Exception $e) {
			$this->__service->rollBack();
	...
	public function reset_detail($paper_id) {
		try {
			$this->__service->begin();
			$this->__service->delete_by_paperid($paper_id);
			$this->__service->commit();
		} catch (Exception $e) {
			$this->__service->rollBack();
			return $this->set_errmsg(voa_errcode_oa_exam::DELETE_PAPER_FAILED);
