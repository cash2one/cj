<?php
/**
 * voa_uda_frontend_goods_table
 * 统一数据访问/商品应用/表格操作
 * $Author$
 * $Id$
 */

class voa_uda_frontend_goods_table extends voa_uda_frontend_goods_abstract {
	...
	public function update($member, $gp, $tid) {
		...
		// 缓存更新
		voa_h_cache::get_instance()->get('goodstable', 'oa', true);
	...
	public function add($member, $gp, &$table) {
		...
		if (array_key_exists($table['tunique'], $this->_tables)) {
			$this->set_errmsg(voa_errcode_oa_goods::GOODS_TUNIQUE_DUPLICATE);
			return false;
		}
	...
	private function __parse_gp($gp, &$table) {
		...
		$fields = array(
			...
			array('tname', self::VAR_STR, 'chk_tname', voa_errcode_oa_goods::GOODS_TABLENAME_IS_EMPTY),
	...
	public function chk_cp_identifier($identifier, $err = null) {
		...
		// 如果插件唯一标识为空
		if (empty($identifier)) {
			$this->set_errmsg(voa_errcode_oa_goods::GOODS_CP_IDENTIFIER_IS_EMPTY);
			return false;
		}
		...
		// 如果该插件标识不存在
		if (false == $is_exists) {
			$this->set_errmsg(voa_errcode_oa_goods::GOODS_CP_IDENTIFIER_IS_NOT_EXIST);
			return false;
		}
	...
	public function chk_tunique($unique, $err = null) {
		...
		// 如果表格唯一标识为空
		if (empty($unique)) {
			$this->set_errmsg(voa_errcode_oa_goods::GOODS_TUNIQUE_IS_EMPTY);
			return false;
		}
