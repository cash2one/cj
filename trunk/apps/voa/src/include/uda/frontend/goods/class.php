...
class voa_uda_frontend_goods_class extends voa_uda_frontend_goods_abstract {
	...
	public function list_all($gp, &$list, $page_option, &$total) {
		...
		if (!empty($query)) {
			$conds['classname LIKE ?'] = "%{$query}%";
		}

		// 获取分类
		$t = new voa_d_oa_goods_class();
		$list = $t->list_by_conds($conds, $page_option);
	...
	public function add($gp, &$class) {
		...
		if ($t->get_by_conds($so_conds)) {
			$this->set_errmsg(voa_errcode_oa_goods::CLASSNAME_DUPLICATE);
			return false;
		}
	...
	public function delete($classid) {
		...
		if ($t->get_by_conds(array('tid=?' => $this->_table['tid'], 'pid=?' => $classid))) {
			$this->set_errmsg(voa_errcode_oa_goods::GOODS_CLASS_HAS_CHILD);
			return false;
		}
	...
	private function __parse_gp($gp, &$data) {
		...
		$fields = array(
			array('classname', self::VAR_STR, 'chk_classname', voa_errcode_oa_goods::GOODS_CLASSNAME_IS_EMPTY, false),
			array('pid', self::VAR_INT, 'chk_classid', voa_errcode_oa_goods::GOODS_CLASSID_ERR, false)
		);
		// 提取数据
