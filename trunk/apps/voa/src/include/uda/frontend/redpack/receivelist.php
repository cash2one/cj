...
class voa_uda_frontend_redpack_receivelist extends voa_uda_frontend_redpack_abstract {
	// 请求的参数
	private $__in = array();
	// 返回的结果集
	private $__out = array();
	...
	*/
	public function doit(array &$out = array(), array $in, $options = array()) {
		...
		// 请求规则定义
		$fields = array(
			array('uid', self::VAR_INT, null, null, true), // 用户uid
		...
		$list = $this->_serv_rplog->list_by_uid($this->__in['uid'], $page_option, array('id' => 'DESC'));
