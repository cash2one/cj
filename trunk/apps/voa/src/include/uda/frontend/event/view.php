...
/**
 * voa_uda_frontend_event_view
 * 应用uda
 * Create By Deepseath
 * $Author$
 * $Id$
 */
class voa_uda_frontend_event_view extends voa_uda_frontend_event_base {

	public function __construct() {
		parent::__construct();
		$this->serv_exterior = &service::factory('voa_s_oa_event_outsider');
		$this->serv_interior = &service::factory('voa_s_oa_event_partake');
	}
	...
	public function doit($request, &$result) {
		...
		$partake = array();
		//活动参与人员信息
		$serv_partake = &service::factory('voa_s_oa_event_partake');
...
//	$data['nowusers'] = $nowusers[$data['m_uid']]['m_username'];
		...
		$serv_nopartake = &service::factory('voa_s_oa_event_nopartake');
	...
	public function qrcode($acid, $npe, $file = '', $is_download = false) {
		...
		$url = $scheme . $sets['domain'] . "/frontend/event/view?acid=" . urlencode($acid) . "&ac=checkin" . "&npe=" . urlencode($npe);
		// 纠错级别：L、M、Q、H
	...
	public function outqrcode($qdata, $file = '', $is_download = false) {
		...
		//跳转地址
		$scheme = config::get('voa.oa_http_scheme');
		$url = $scheme . $sets['domain'] . "/frontend/event/outqcode?acid=" . urlencode($qdata['acid']) . "&ac=check&outname=" . urlencode($qdata['outname']) . "&outphone=" . urlencode($qdata['outphone']);
		// 纠错级别：L、M、Q、H
		$errorCorrectionLevel = 'L';
		// 点的大小：1到10
		$matrixPointSize = 10;
	...
	public function unread_list($acid) {


		$join_list = $this->serv_interior->list_by_conds($conds);
		print_r($join_list);exit;
	}
