...
/**
 * voa_uda_frontend_nvote_mem_option_get
 * 获取用户投票
 * User: luckwang
 * Date: 15/3/9
 * Time: 下午4:55
...
class voa_uda_frontend_nvote_mem_option_get extends voa_uda_frontend_nvote_mem_option_abstract {
     ...
     * @return bool
     */
    public function mem_option_list($nvote_id, &$result) {
        $nvote_id = rintval($nvote_id);
        if ($nvote_id < 1) {
            return false;
        }

        $result = $this->_serv->list_by_conds(array('nvote_id =?' => $nvote_id));
        if (!$result) {
            $result = array();
        }
        return true;
