...
/**
 * voa_uda_frontend_activity_sign
 * 应用uda
 * Create By Deepseath
 * $Author$
 * $Id$
 */
class voa_uda_frontend_activity_sign extends voa_uda_frontend_activity_base {
	...
	public function __construct() {
		parent::__construct();
		$this->serv = &service::factory('voa_s_oa_activity');
		$this->serv_invite = &service::factory('voa_s_oa_activity_invite');
		$this->serv_partake = &service::factory('voa_s_oa_activity_partake');
		$this->serv_nopartake = &service::factory('voa_s_oa_activity_nopartake');
		$this->serv_outsider = &service::factory('voa_s_oa_activity_outsider');
	}
		...
		switch ($request['ac']) {
			case 'join'://报名
				...
				if ($res == 'true') {
					...
					$result = array('url' => '/frontend/activity/view/?pluginid=' . startup_env::get('pluginid') . '&acid=' . $request['acid'] . '&npe=' . $npe, 'message' => "报名成功");
					...
					$msg_url = $scheme . $request['_setting']['domain'] . '/frontend/activity/view/?acid=' . $servdata['acid'] . '&pluginid=' . startup_env::get('pluginid');
					try {
						if (!empty($m_uidto)) {
			...
			case 'apply'://申请取消
				...
				if (empty($request['message'])) {
					$result = array('url' => '/frontend/activity/view/?pluginid=' . startup_env::get('pluginid') . '&acid=' . $request['acid'], 'message' => "请输入退出原因");
					break;
				}
				$partake = $this->serv_partake->get_by_conds(array('acid' => $request['acid'], 'm_uid' => $request['m_uid']));
				// 判断是否已经签到，完成签到不能取消
				if ($partake['check'] == '1') {
					$result = array('url' => '/frontend/activity/view/?pluginid=' . startup_env::get('pluginid') . '&acid=' . $request['acid'], 'message' => "签到后不能退出");
					break;
				}
				$apid = $partake['apid'];
				...
				$result = array('url' => '/frontend/activity/view/?pluginid=' . startup_env::get('pluginid') . '&acid=' . $request['acid'], 'message' => "已发送申请取消");
				/** 发送微信消息 */
				$servdata = $this->serv->get($request['acid']);
				$m_uidto = $servdata['m_uid'];
				...
				$msg_desc = "主题：【" . $servdata['title'] . "】\n";
				$msg_desc .= "活动时间：" . rgmdate($servdata['start_time'], "m-d H:i") . " 到 " . rgmdate($servdata['end_time'], "m-d H:i") . "\n";
				$msg_url = $scheme . $request['_setting']['domain'] . '/frontend/activity/return/?apid=' . $apid . '&acid=' . $partake['acid'] . '&pluginid=' . startup_env::get('pluginid');
				try {
					if (!empty($m_uidto)) {
			...
			case 'view'://显示数据
				...
				$activity = $this->serv->get_by_conds(array('acid' => $acid));
				$result['title'] = $activity['title'];
				$result['acid'] = $activity['acid'];
				$result['apid'] = $apid;
			...
			case 'reject'://驳回
				...
				$msg_url = $scheme . $request['_setting']['domain'] . '/frontend/activity/view/?acid=' . $acid . '&pluginid=' . startup_env::get('pluginid');
				try {
					if (!empty($m_uid)) {
				...
				$result = array('url' => '/frontend/activity/view/?pluginid=' . startup_env::get('pluginid') . '&acid=' . $acid, 'message' => "驳回成功");
				break;
			case 'agree'://同意
				$apid = $request['apid'];
				$partake = $this->serv_partake->get_by_conds(array('apid' => $apid));
				...
				$result = array('url' => '/frontend/activity/view/?pluginid=' . startup_env::get('pluginid') . '&acid=' . $acid, 'message' => "同意成功");
				break;
		}
		return true;
	}
