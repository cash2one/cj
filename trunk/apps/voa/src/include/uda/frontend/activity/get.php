...
/**
 * voa_uda_frontend_activity_get
 * 应用uda
 * Create By Deepseath
 * $Author$
 * $Id$
 */
class voa_uda_frontend_activity_get extends voa_uda_frontend_activity_base {
	...
	public function getact($in, &$out) {
		...
		if (!isset($data['acid']) && $data == '' && $data['acid'] != $in['acid']) {
			return false;
		} else {
			$out = $data;
		}
	 ...
	 * @return bool
	 */
	public function handle($in, &$out) {
		$out = array(
			'title' => $in['title'],
			'content' => $in['content'],
			'address' => $in['address'],
			'start_data' => rgmdate($in['start_time'], "Y-m-d"),
			'start_time' => rgmdate($in['start_time'], "H:i"),
			'end_data' => rgmdate($in['end_time'], "Y-m-d"),
			'end_time' => rgmdate($in['end_time'], "H:i"),
			'cut_off_data' => rgmdate($in['cut_off_time'], "Y-m-d"),
			'cut_off_time' => rgmdate($in['cut_off_time'], "H:i"),
		);
		return true;
	}
