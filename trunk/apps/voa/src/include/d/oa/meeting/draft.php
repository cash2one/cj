...
class voa_d_oa_meeting_draft extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.meeting_draft';
	/** 主键 */
	private static $__pk = 'mtd_id';
	/** 所有字段名 */
	private static $__fields = array(
		'mtd_id', 'm_openid', 'mtd_message',
		'mtd_status', 'mtd_created', 'mtd_updated', 'mtd_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `mtd_status`<'%d'
			ORDER BY `mtd_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `mtd_id`='%d' AND `mtd_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `mtd_id` IN (%n) AND `mtd_status`<'%d'
			ORDER BY `mtd_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND mtd_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND mtd_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['mtd_status'])) {
			$data['mtd_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['mtd_created'])) {
			$data['mtd_created'] = startup_env::get('timestamp');
		}

		if (empty($data['mtd_updated'])) {
			$data['mtd_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'mtd_status' => self::STATUS_REMOVE,
			'mtd_deleted' => startup_env::get('timestamp')
	...
	public static function get_by_openid($openid, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `m_openid`=%s AND `mtd_status`<'%d' ORDER BY mtd_id DESC", array(
