...
class voa_d_oa_askoff extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.askoff';
	/** 主键 */
	private static $__pk = 'ao_id';
	/** 所有字段名 */
	private static $__fields = array(
			'ao_id', 'm_uid', 'm_username', 'ao_subject', 'aopc_id',
			'ao_status', 'ao_created', 'ao_updated', 'ao_deleted',
			'ao_begintime', 'ao_endtime', 'ao_type'
	);
	/** 审批中 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `ao_status`<'%d'
			ORDER BY `ao_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `ao_id`='%d' AND `ao_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `ao_id` IN (%n) AND `ao_status`<'%d'
			ORDER BY `ao_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND ao_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND ao_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['ao_status'])) {
			$data['ao_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['ao_created'])) {
			$data['ao_created'] = startup_env::get('timestamp');
		}

		if (empty($data['ao_updated'])) {
			$data['ao_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'ao_status' => self::STATUS_REMOVE,
			'ao_deleted' => startup_env::get('timestamp')
