<?php
/**
 * voa_d_oa_askfor_attachment
 * 应用.审批/附件表
 * Create By Deepseath
 * $Author$
 * $Id$
 */
class voa_d_oa_askfor_attachment extends dao_mysql {

	/** 表名 */
	public static $__table = 'oa.askfor_attachment';
	/** 主键 */
	private static $__pk = 'afat_id';
	/** 字段前缀 */
	private static $__prefix = 'afat_';
	/** 所有字段名 */
	private static $__fields = array(
		'af_id', 'afc_id', 'at_id',
		'm_id', 'm_username',
		'afat_status', 'afat_created', 'afat_updated', 'afat_deleted'
	);
	 ...
	 * @return array
	 */
	public static function fetch_all_by_af_id($af_id, $shard_key = array()) {
		$af_id = (int)$af_id;
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t WHERE %i AND %i ORDER BY `afat_id` DESC", array(
			self::$__table, db_help::field('af_id', $af_id), db_help::field('afat_status', self::STATUS_REMOVE, '<')
		), self::$__pk, $shard_key);
	}
