...
class voa_d_oa_askfor_setting extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.askfor_setting';
	/** 主键 */
	private static $__pk = 'afs_key';
	/** 正常 */
	...
	public static function fetch_all($shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `afs_status`<'%d' ORDER BY `afs_key` ASC", array(
	...
	public static function insert($data, $shard_key = array()) {
		if (empty($data['afs_status'])) {
			$data['afs_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['afs_created'])) {
			$data['afs_created'] = startup_env::get('timestamp');
		}
	...
	public static function update($data, $shard_key = array()) {
		if (empty($data['afs_status'])) {
			$data['afs_status'] = self::STATUS_UPDATE;
		}

		if (empty($data['afs_updated'])) {
			$data['afs_updated'] = startup_env::get('timestamp');
		}

		return parent::_update(self::$__table, $data, array('afs_key' => $data['afs_key']), false, false, $shard_key);
	}

	/** 删除数据 */
	public static function delete($cachenames, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'afs_status' => self::STATUS_REMOVE,
			'afs_deleted' => startup_env::get('timestamp')
		), db_help::field('afs_key', $cachenames), false, false, $shard_key);
	}
