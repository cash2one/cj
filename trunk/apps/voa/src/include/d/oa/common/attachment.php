...
class voa_d_oa_common_attachment extends dao_mysql {
	...
	/** 表名 */
	public static $__table = 'oa.common_attachment';
	/** 主键 */
	private static $__pk = 'at_id';
	//所有字段名
	private static $__fields = array(
		'at_id', 'm_uid', 'm_username', 'at_filename', 'at_filesize', 'at_attachment', 'at_remote', 'at_description',
		'at_isimage', 'at_width', 'at_thumb', 'at_status', 'at_created', 'at_updated', 'at_deleted','at_isattach'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `at_status`<'%d'
			ORDER BY `at_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `at_id`='%d' AND `at_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `at_id` IN (%n) AND `at_status`<'%d'
			ORDER BY `at_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND at_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND at_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['at_status'])) {
			$data['at_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['at_created'])) {
			$data['at_created'] = startup_env::get('timestamp');
		}

		if (empty($data['at_updated'])) {
			$data['at_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'at_status' => self::STATUS_REMOVE,
			'at_deleted' => startup_env::get('timestamp')
