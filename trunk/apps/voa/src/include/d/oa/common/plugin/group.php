...
class voa_d_oa_common_plugin_group extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.common_plugin_group';
	/** 主键 */
	private static $__pk = 'cpg_id';
	/** 所有字段名 */
	private static $__fields = array(
			'cpg_id', 'cpg_name', 'cpg_ordernum',
			'cpg_status', 'cpg_created', 'cpg_updated', 'cpg_deleted'
	);
	/** 审批中 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `cpg_status`<'%d'
			ORDER BY `cpg_ordernum` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `cpg_id`='%d' AND `cpg_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `cpg_id` IN (%n) AND `cpg_status`<'%d'
			ORDER BY `cpg_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND cpg_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND cpg_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['cpg_status'])) {
			$data['cpg_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['cpg_created'])) {
			$data['cpg_created'] = startup_env::get('timestamp');
		}

		if (empty($data['cpg_updated'])) {
			$data['cpg_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'cpg_status' => self::STATUS_REMOVE,
			'cpg_deleted' => startup_env::get('timestamp')
