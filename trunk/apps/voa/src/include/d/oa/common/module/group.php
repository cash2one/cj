 ...
 * $Id$
 */
class voa_d_oa_common_module_group extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.common_module_group';
	/** 主键 */
	private static $__pk = 'cmg_id';
	/** 字段前缀 */
	private static $__prefix = 'cmg_';
	/** 正常 */
	 ...
	 * @return array
	 */
	public static function fetch_by_cmg_dir($cmg_dir, $shard_key = array()) {
		return (array) parent::_fetch_first(self::$__table, "SELECT * FROM %t WHERE %i AND %i LIMIT 1", array(
			self::$__table, db_help::field('cmg_dir', $cmg_dir), db_help::field('cmg_status', self::STATUS_REMOVE, '<')
		), $shard_key);
	}
}
