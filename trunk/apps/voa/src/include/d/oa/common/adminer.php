...
class voa_d_oa_common_adminer extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.common_adminer';
	/** 主键 */
	 ...
	 */
	public static function count_by_mobilephone_notid($mobilephone, $ca_id = 0, $shard_key = array()) {
		return parent::_result_first(self::$__table, "SELECT COUNT(`ca_mobilephone`) FROM %t WHERE %i AND %i AND %i", array(
			self::$__table, db_help::field('ca_mobilephone', $mobilephone), db_help::field('ca_status', self::STATUS_REMOVE, '<'), db_help::field('ca_id', $ca_id, '<>')
		));
	}
	 ...
	 * @return Ambigous <void, boolean>
	 */
	public static function count_by_email_notid($email, $ca_id = 0, $shard_key = array()) {
		return parent::_result_first(self::$__table, "SELECT COUNT(`ca_email`) FROM %t WHERE %i AND %i AND %i", array(
			self::$__table, db_help::field('ca_email', $email), db_help::field('ca_status', self::STATUS_REMOVE, '<'), db_help::field('ca_id', $ca_id, '<>')
		));
	}
	 ...
	 * @return boolean
	 */
	public static function fetch_by_mobilephone($mobilephone, $shard_key = array()) {
		if (empty($mobilephone)) {
			return array();
		}
		return parent::_fetch_first(self::$__table,
				"SELECT * FROM %t WHERE `ca_mobilephone`=%s AND `ca_status`<'%d' LIMIT 1", array(
					self::$__table, $mobilephone, self::STATUS_REMOVE
				), $shard_key
		);
	}
	...
	public static function fetch_by_email($email, $shard_key = array()) {
		...
		return parent::_fetch_first(self::$__table,
				"SELECT * FROM %t WHERE `ca_email`=%s AND `ca_status`<'%d' LIMIT 1", array(
	 ...
	 * @param int $ca_id 排除这个人的ID
	 * @param array $shard_key
	 * @throws dao_exception
	 * @throws db_exception
	 */
	public static function count_by_cag_id($cag_id, $ca_id = 0, $shard_key = array()) {
		return parent::_result_first(self::$__table, "SELECT COUNT(`cag_id`) FROM %t WHERE %i AND %i AND %i", array(
			self::$__table, db_help::field('cag_id', $cag_id), db_help::field('ca_status', self::STATUS_REMOVE, '<'), db_help::field('ca_id', $ca_id, '<>')
		));
	}
