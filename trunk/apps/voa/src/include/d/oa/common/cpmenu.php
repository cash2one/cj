...
class voa_d_oa_common_cpmenu extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.common_cpmenu';
	/** 主键 */
	...
	public static function fetch_all($start = 0, $limit = 0, $force = false, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t WHERE %i ORDER BY `ccm_displayorder` ASC ".db_help::limit($start, $limit),array(
	 ...
	 */
	public static function update_by_cp_pluginid_ccm_module($data, $cp_pluginid, $shard_key = array()) {
		$conditions = array();
		$conditions[] = db_help::field('cp_pluginid', $cp_pluginid);
		if (empty($data[self::fieldname('status')])) {
			$data[self::fieldname('status')] = self::STATUS_UPDATE;
		}
		if (empty($data[self::fieldname('update')])) {
		...
		return parent::_update(self::$__table, $data, implode(' AND ', $conditions), false, false, $shard_key);
	}
