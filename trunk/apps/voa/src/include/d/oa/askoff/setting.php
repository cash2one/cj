...
class voa_d_oa_askoff_setting extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.askoff_setting';
	/** 主键 */
	private static $__pk = 'aos_key';
	/** 所有字段名 */
	private static $__fields = array(
		'aos_key', 'aos_value', 'aos_type', 'aos_comment',
		'aos_created', 'aos_updated', 'aos_deleted'
	);
	/** 正常 */
	...
	public static function fetch_by_pk($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE %i AND %i LIMIT 1", array(
				self::$__table, db_help::field(self::$__pk, $id), db_help::field('aos_status', self::STATUS_REMOVE, '<')
			), $shard_key
		);
	}
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND aos_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND mrs_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['aos_status'])) {
			$data['aos_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['aos_created'])) {
			$data['aos_created'] = startup_env::get('timestamp');
		}

		if (empty($data['aos_updated'])) {
			$data['aos_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_pks($pks, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'aos_status' => self::STATUS_REMOVE,
			'aos_deleted' => startup_env::get('timestamp')
