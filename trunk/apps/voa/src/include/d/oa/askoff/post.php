...
class voa_d_oa_askoff_post extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.askoff_post';
	/** 主键 */
	private static $__pk = 'aopt_id';
	/** 所有字段名 */
	private static $__fields = array(
		'aopt_id', 'm_uid', 'm_username', 'ao_id', 'aopt_subject', 'aopt_message', 'aopt_first',
		'aopt_status', 'aopt_created', 'aopt_updated', 'aopt_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `aopt_status`<'%d'
			ORDER BY `aopt_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `aopt_id`='%d' AND `aopt_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `aopt_id` IN (%n) AND `aopt_status`<'%d'
			ORDER BY `aopt_id` DESC", array(
	 ...
	 */
	public static function fetch_first_by_ao_id($ao_id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE ao_id=%d AND aopt_first=1 AND aopt_status<%d", array(
				self::$__table, $ao_id, self::STATUS_REMOVE
			), $shard_key
		);
	}
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND aopt_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND aopt_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['aopt_status'])) {
			$data['aopt_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['aopt_created'])) {
			$data['aopt_created'] = startup_env::get('timestamp');
		}

		if (empty($data['aopt_updated'])) {
			$data['aopt_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'aopt_status' => self::STATUS_REMOVE,
			'aopt_deleted' => startup_env::get('timestamp')
