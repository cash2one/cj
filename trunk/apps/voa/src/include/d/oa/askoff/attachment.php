<?php
/**
 * voa_d_oa_askoff_attachment
 * 应用/请假/附件表
 * Create By Deepseath
 * $Author$
 * $Id$
 */
class voa_d_oa_askoff_attachment extends dao_mysql {

	/** 表名 */
	public static $__table = 'oa.askoff_attachment';
	/** 主键 */
	private static $__pk = 'aoat_id';
	/** 字段前缀 */
	private static $__prefix = 'aoat_';
	/** 所有字段名 */
	private static $__fields = array(
		'aoat_id', 'ao_id', 'aopt_id', 'at_id',
		'm_uid', 'm_username',
		'aoat_status', 'aoat_created', 'aoat_upatated', 'aoat_deleted'
	);
	 ...
	 * @return array
	 */
	public static function fetch_all_by_ao_id($ao_id, $shard_key = array()) {
		$ao_id = (int)$ao_id;
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t WHERE %i AND %i ORDER BY `aoat_id` DESC", array(
			self::$__table, db_help::field('ao_id', $ao_id), db_help::field('aoat_status', self::STATUS_REMOVE, '<')
		), self::$__pk, $shard_key);
	}
