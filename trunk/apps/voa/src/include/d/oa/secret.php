...
class voa_d_oa_secret extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.secret';
	/** 主键 */
	private static $__pk = 'st_id';
	/** 所有字段名 */
	private static $__fields = array(
		'st_id', 'm_uid', 'm_username', 'st_subject',
		'st_status', 'st_created', 'st_updated', 'st_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `st_status`<'%d'
			ORDER BY `st_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function count_all($shard_key = array()) {
		return (int) parent::_result_first(self::$__table, "SELECT COUNT(`st_id`) FROM %t WHERE %i", array(
				self::$__table, db_help::field('st_status', self::STATUS_REMOVE, '<')
		), $shard_key);
	}
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `st_id`='%d' AND `st_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `st_id` IN (%n) AND `st_status`<'%d'
			ORDER BY `st_id` DESC", array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['st_status'])) {
			$data['st_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['st_created'])) {
			$data['st_created'] = startup_env::get('timestamp');
		}

		if (empty($data['st_updated'])) {
			$data['st_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'st_status' => self::STATUS_REMOVE,
			'st_deleted' => startup_env::get('timestamp')
