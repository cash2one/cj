...
class voa_d_oa_member_share extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.member_share';
	/** 主键 */
	private static $__pk = 'msh_id';
	/** 所有字段名 */
	private static $__fields = array(
		'msh_id', 'm_uid', 'msh_fields', 'msh_status', 'msh_created', 'msh_updated', 'msh_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `msh_status`<'%d'
			ORDER BY `msh_ordernum` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `msh_id`='%d' AND `msh_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `msh_id` IN (%n) AND `msh_status`<'%d'
			ORDER BY `msh_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND msh_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND msh_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['msh_status'])) {
			$data['msh_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['msh_created'])) {
			$data['msh_created'] = startup_env::get('timestamp');
		}

		if (empty($data['msh_updated'])) {
			$data['msh_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'msh_status' => self::STATUS_REMOVE,
			'msh_deleted' => startup_env::get('timestamp')
	...
	public static function delete_by_m_uid($m_uid, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'msh_status' => self::STATUS_REMOVE,
			'msh_deleted' => startup_env::get('timestamp')
		), db_help::field('m_uid', $m_uid), false, $shard_key);
	}
