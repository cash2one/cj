<?php
/**
 * voa_d_oa_project_attachment
 * 应用.任务/附件表
 * Create By Deepseath
 * $Author$
 * $Id$
 */
class voa_d_oa_project_attachment extends dao_mysql {

	/** 表名 */
	public static $__table = 'oa.project_attachment';
	/** 主键 */
	private static $__pk = 'pat_id';
	/** 字段前缀 */
	private static $__prefix = 'pat_';
	/** 所有字段名 */
	private static $__fields = array(
		'p_id', 'pp_id', 'at_id',
		'm_id', 'm_username',
		'pat_status', 'pat_created', 'pat_upatated', 'pat_deleted'
	);
	 ...
	 * @return array
	 */
	public static function fetch_all_by_p_id($p_id, $shard_key = array()) {
		$p_id = (int)$p_id;
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t WHERE %i AND %i ORDER BY `pat_id` DESC", array(
			self::$__table, db_help::field('p_id', $p_id), db_help::field('pat_status', self::STATUS_REMOVE, '<')
		), self::$__pk, $shard_key);
	}
