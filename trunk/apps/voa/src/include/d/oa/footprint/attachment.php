...
class voa_d_oa_footprint_attachment extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.footprint_attachment';
	/** 主键 */
	private static $__pk = 'fpat_id';
	/** 所有字段名 */
	private static $__fields = array(
		'fpat_id', 'm_uid', 'm_username', 'fp_id', 'at_id',
		'fpat_status', 'fpat_created', 'fpat_updated', 'fpat_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `fpat_status`<'%d'
			ORDER BY `fpat_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `fpat_id`='%d' AND `fpat_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `fpat_id` IN (%n) AND `fpat_status`<'%d'
			ORDER BY `fpat_id` DESC", array(
	...
	public static function fetch_by_fp_id($fp_id, $start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT a.fp_id, b.* FROM %t AS a
			LEFT JOIN %t AS b ON a.at_id=b.at_id
			WHERE a.fp_id IN (%n) AND a.fpat_status<%d
			ORDER BY a.at_id DESC", array(
				self::$__table, voa_d_oa_common_attachment::$__table, (array)$fp_id, self::STATUS_REMOVE
			), self::$__pk, $shard_key
		);
	}
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND fpat_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND fpat_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['fpat_status'])) {
			$data['fpat_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['fpat_created'])) {
			$data['fpat_created'] = startup_env::get('timestamp');
		}

		if (empty($data['fpat_updated'])) {
			$data['fpat_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'fpat_status' => self::STATUS_REMOVE,
			'fpat_deleted' => startup_env::get('timestamp')
