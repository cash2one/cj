...
class voa_d_oa_footprint_post extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.footprint_post';
	/** 主键 */
	private static $__pk = 'fppt_id';
	/** 所有字段名 */
	private static $__fields = array(
		'fppt_id', 'fp_id', 'm_uid', 'm_username', 'fppt_subject', 'fppt_message', 'fppt_first',
		'fppt_status', 'fppt_created', 'fppt_updated', 'fppt_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `fppt_status`<'%d'
			ORDER BY `fppt_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `fppt_id`='%d' AND `fppt_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `fppt_id` IN (%n) AND `fppt_status`<'%d'
			ORDER BY `fppt_id` DESC", array(
	...
	/**
	 * 根据 fp_id 读取回复的信息
	 * @param array $fp_id 会议纪要id
	 * @param number $start
	 * @param number $limit
	 * @return Ambigous <multitype:, array>
	 */
	public static function fetch_by_fp_id($fp_id, $start = 0, $limit = 0, $shard_key = array()) {
		return self::fetch_by_conditions(array('fp_id' => array($fp_id)), $start, $limit, $shard_key);
	}
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND fppt_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND fppt_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['fppt_status'])) {
			$data['fppt_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['fppt_created'])) {
			$data['fppt_created'] = startup_env::get('timestamp');
		}

		if (empty($data['fppt_updated'])) {
			$data['fppt_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'fppt_status' => self::STATUS_REMOVE,
			'fppt_deleted' => startup_env::get('timestamp')
