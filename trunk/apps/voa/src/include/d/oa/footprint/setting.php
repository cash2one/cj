...
class voa_d_oa_footprint_setting extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.footprint_setting';
	/** 主键 */
	private static $__pk = 'fps_key';
	/** 所有字段名 */
	private static $__fields = array(
		'fps_key', 'fps_value', 'fps_type', 'fps_comment',
		'fps_created', 'fps_updated', 'fps_deleted'
	);
	/** 正常 */
	...
	public static function fetch_by_pk($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `%i`='%d' AND `fps_status`<'%d'", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND fps_status<%d", array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['fps_status'])) {
			$data['fps_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['fps_created'])) {
			$data['fps_created'] = startup_env::get('timestamp');
		}

		if (empty($data['fps_updated'])) {
			$data['fps_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_pks($pks, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'fps_status' => self::STATUS_REMOVE,
			'fps_deleted' => startup_env::get('timestamp')
