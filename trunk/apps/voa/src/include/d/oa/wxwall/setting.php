...
class voa_d_oa_wxwall_setting extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.wxwall_setting';
	/** 主键 */
	private static $__pk = 'wws_id';
	/** 正常 */
	...
	public static function insert($data, $shard_key = array()) {
		if (empty($data['wws_status'])) {
			$data['wws_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['wws_created'])) {
			$data['wws_created'] = startup_env::get('timestamp');
		}
	...
	public static function update($data, $shard_key = array()) {
		if (empty($data['wws_status'])) {
			$data['wws_status'] = self::STATUS_UPDATE;
		}

		if (empty($data['wws_updated'])) {
			$data['wws_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete($cachenames, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'wws_status' => self::STATUS_REMOVE,
			'wws_deleted' => startup_env::get('timestamp')
