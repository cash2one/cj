...
class voa_d_oa_datum extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.datum';
	/** 主键 */
	private static $__pk = 'dt_id';
	/** 所有字段名 */
	private static $__fields = array(
		'dt_id', 'm_uid', 'm_username', 'dtf_id', 'dt_remark', 'dt_displayorder',
		'dt_status', 'dt_created', 'dt_updated', 'dt_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `dt_status`<'%d'
			ORDER BY `dt_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `dt_id`='%d' AND `dt_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `dt_id` IN (%n) AND `dt_status`<'%d'
			ORDER BY `dt_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND dt_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND dt_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['dt_status'])) {
			$data['dt_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['dt_created'])) {
			$data['dt_created'] = startup_env::get('timestamp');
		}

		if (empty($data['dt_updated'])) {
			$data['dt_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'dt_status' => self::STATUS_REMOVE,
			'dt_deleted' => startup_env::get('timestamp')
