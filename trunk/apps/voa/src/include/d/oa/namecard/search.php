...
class voa_d_oa_namecard_search extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.namecard_search';
	/** 主键 */
	private static $__pk = 'ncso_id';
	/** 所有字段名 */
	private static $__fields = array(
		'ncso_id', 'nc_id', 'm_uid', 'ncso_message',
		'ncso_status', 'ncso_created', 'ncso_updated', 'ncso_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `ncso_status`<'%d'
			ORDER BY `ncso_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `ncso_id`='%d' AND `ncso_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `ncso_id` IN (%n) AND `ncso_status`<'%d'
			ORDER BY `ncso_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND ncso_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND ncso_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['ncso_status'])) {
			$data['ncso_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['ncso_created'])) {
			$data['ncso_created'] = startup_env::get('timestamp');
		}

		if (empty($data['ncso_updated'])) {
			$data['ncso_updated'] = startup_env::get('timestamp');
		}
