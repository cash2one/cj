...
class voa_d_oa_namecard_setting extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.namecard_setting';
	/** 主键 */
	private static $__pk = 'ncs_key';
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `ncs_status`<'%d'
			ORDER BY `ncs_key` ASC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $shard_key = array()) {
		if (empty($data['ncs_status'])) {
			$data['ncs_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['ncs_created'])) {
			$data['ncs_created'] = startup_env::get('timestamp');
		}
	...
	public static function update($data, $shard_key = array()) {
		if (empty($data['ncs_status'])) {
			$data['ncs_status'] = self::STATUS_UPDATE;
		}

		if (empty($data['ncs_updated'])) {
			$data['ncs_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete($cachenames, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'ncs_status' => self::STATUS_REMOVE,
			'ncs_deleted' => startup_env::get('timestamp')
