...
class voa_d_oa_plan extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.plan';
	/** 主键 */
	private static $__pk = 'pl_id';
	/** 所有字段名 */
	private static $__fields = array(
		'pl_id', 'm_uid', 'm_username', 'pl_subject', 'pl_type',
		'pl_begin_at', 'pl_finish_at', 'pl_alarm_at', 'pl_status',
		'pl_created', 'pl_updated', 'pl_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `pl_status`<'%d'
			ORDER BY `pl_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `pl_id`='%d' AND `pl_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `pl_id` IN (%n) AND `pl_status`<'%d'
			ORDER BY `pl_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int) parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND pl_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array) parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND pl_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['pl_status'])) {
			$data['pl_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['pl_created'])) {
			$data['pl_created'] = startup_env::get('timestamp');
		}

		if (empty($data['pl_updated'])) {
			$data['pl_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'pl_status' => self::STATUS_REMOVE,
			'pl_deleted' => startup_env::get('timestamp')
