...
class voa_d_oa_vnote_post extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.vnote_post';
	/** 主键 */
	private static $__pk = 'vnp_id';
	/** 所有字段名 */
	private static $__fields = array(
		'vnp_id', 'm_uid', 'm_username', 'vn_id', 'vnp_subject', 'vnp_message', 'vnp_first',
		'vnp_status', 'vnp_created', 'vnp_updated', 'vnp_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `vnp_status`<'%d'
			ORDER BY `vnp_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `vnp_id`='%d' AND `vnp_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `vnp_id` IN (%n) AND `vnp_status`<'%d'
			ORDER BY `vnp_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND vnp_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND vnp_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['vnp_status'])) {
			$data['vnp_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['vnp_created'])) {
			$data['vnp_created'] = startup_env::get('timestamp');
		}

		if (empty($data['vnp_updated'])) {
			$data['vnp_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'vnp_status' => self::STATUS_REMOVE,
			'vnp_deleted' => startup_env::get('timestamp')
		), db_help::field(self::$__pk, $ids), $unbuffered, false, $shard_key);
	}

	/**
	 * 删除指定vn_id（单个或多个）的日报相关批注
