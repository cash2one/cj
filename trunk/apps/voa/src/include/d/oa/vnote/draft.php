...
class voa_d_oa_vnote_draft extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.vnote_draft';
	/** 主键 */
	private static $__pk = 'vnd_id';
	/** 所有字段名 */
	private static $__fields = array(
		'vnd_id', 'm_openid', 'vnd_message',
		'vnd_status', 'vnd_created', 'vnd_updated', 'vnd_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `vnd_status`<'%d'
			ORDER BY `vnd_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `vnd_id`='%d' AND `vnd_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `vnd_id` IN (%n) AND `vnd_status`<'%d'
			ORDER BY `vnd_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND vnd_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND vnd_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['vnd_status'])) {
			$data['vnd_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['vnd_created'])) {
			$data['vnd_created'] = startup_env::get('timestamp');
		}

		if (empty($data['vnd_updated'])) {
			$data['vnd_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'vnd_status' => self::STATUS_REMOVE,
			'vnd_deleted' => startup_env::get('timestamp')
	...
	public static function get_by_openid($openid, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `m_openid`=%s AND `vnd_status`<'%d' ORDER BY vnd_id DESC", array(
