...
class voa_d_oa_vnote_setting extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.vnote_setting';
	/** 主键 */
	private static $__pk = 'vns_key';
	/** 所有字段名 */
	private static $__fields = array(
		'vns_key', 'vns_value', 'vns_type', 'vns_comment',
		'vns_created', 'vns_updated', 'vns_deleted'
	);
	/** 正常 */
	...
	public static function fetch_by_pk($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `%i`='%d' AND `vns_status`<'%d'", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND vns_status<%d", array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['vns_status'])) {
			$data['vns_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['vns_created'])) {
			$data['vns_created'] = startup_env::get('timestamp');
		}

		if (empty($data['vns_updated'])) {
			$data['vns_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_pks($pks, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'vns_status' => self::STATUS_REMOVE,
			'vns_deleted' => startup_env::get('timestamp')
