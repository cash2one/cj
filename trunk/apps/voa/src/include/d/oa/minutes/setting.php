...
class voa_d_oa_minutes_setting extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.minutes_setting';
	/** 主键 */
	private static $__pk = 'mis_key';
	/** 所有字段名 */
	private static $__fields = array(
		'mis_key', 'mis_value', 'mis_type', 'mis_comment',
		'mis_created', 'mis_updated', 'mis_deleted'
	);
	/** 正常 */
	...
	public static function fetch_by_pk($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `%i`='%d' AND `mis_status`<'%d'", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND mis_status", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND mis_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['mis_status'])) {
			$data['mis_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['mis_created'])) {
			$data['mis_created'] = startup_env::get('timestamp');
		}

		if (empty($data['mis_updated'])) {
			$data['mis_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_pks($pks, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'mis_status' => self::STATUS_REMOVE,
			'mis_deleted' => startup_env::get('timestamp')
