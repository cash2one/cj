...
class voa_d_oa_minutes_draft extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.minutes_draft';
	/** 主键 */
	private static $__pk = 'mid_id';
	/** 所有字段名 */
	private static $__fields = array(
		'mid_id', 'm_openid', 'mid_subject', 'mid_message',
		'mid_status', 'mid_created', 'mid_updated', 'mid_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `mid_status`<'%d'
			ORDER BY `mid_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `mid_id`='%d' AND `mid_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `mid_id` IN (%n) AND `mid_status`<'%d'
			ORDER BY `mid_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND mid_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND mid_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['mid_status'])) {
			$data['mid_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['mid_created'])) {
			$data['mid_created'] = startup_env::get('timestamp');
		}

		if (empty($data['mid_updated'])) {
			$data['mid_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'mid_status' => self::STATUS_REMOVE,
			'mid_deleted' => startup_env::get('timestamp')
	...
	public static function get_by_openid($openid, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `m_openid`=%s AND `mid_status`<'%d' ORDER BY mid_id DESC", array(
