<?php
/**
 * voa_d_oa_minutes_attachment
 * 应用/会议记录/附件表
 * Create By Deepseath
 * $Author$
 * $Id$
 */
class voa_d_oa_minutes_attachment extends dao_mysql {

	/** 表名 */
	public static $__table = 'oa.minutes_attachment';
	/** 主键 */
	private static $__pk = 'miat_id';
	/** 字段前缀 */
	private static $__prefix = 'miat_';
	/** 所有字段名 */
	private static $__fields = array(
		'miat_id', 'mi_id', 'mip_id', 'at_id',
		'm_uid', 'm_username',
		'miat_status', 'miat_created', 'miat_upatated', 'miat_deleted'
	);
	 ...
	 * @return array
	 */
	public static function fetch_all_by_mi_id($mi_id, $shard_key = array()) {
		$mi_id = (int)$mi_id;
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t WHERE %i AND %i ORDER BY `miat_id` DESC", array(
			self::$__table, db_help::field('mi_id', $mi_id), db_help::field('miat_status', self::STATUS_REMOVE, '<')
		), self::$__pk, $shard_key);
	}
