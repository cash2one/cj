...
class voa_d_oa_minutes_mem extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.minutes_mem';
	/** 主键 */
	private static $__pk = 'mim_id';
	/** 所有字段名 */
	private static $__fields = array(
		'mim_id', 'm_uid', 'm_username', 'mi_id',
		'mim_status', 'mim_created', 'mim_updated', 'mim_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `mim_status`<'%d'
			ORDER BY `mim_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `mim_id`='%d' AND `mim_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `mim_id` IN (%n) AND `mim_status`<'%d'
			ORDER BY `mim_id` DESC", array(
	 ...
	 * @return Ambigous <multitype:, array>
	 */
	public static function fetch_by_mi_id_uid($mi_id, $uid, $start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `mi_id`=%d AND m_uid=%d AND `mim_status`<%d", array(
				self::$__table, $mi_id, $uid, self::STATUS_REMOVE
			), $shard_key
		);
	}
	...
	public static function fetch_by_search($uid, $conditions, $start = 0, $limit = 0, $shard_key = array()) {
		...
		array_unshift($params, self::$__table, voa_d_oa_minutes::$__table);

		return self::_fetch_all(self::$__table, "SELECT a.* FROM %t AS a
			LEFT JOIN %t AS b ON a.mi_id=b.mi_id
			WHERE {$wherestr} ORDER BY a.mi_id DESC LIMIT $start, $limit", $params, self::$__pk, $shard_key
		);
	}
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND mim_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND mim_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['mim_status'])) {
			$data['mim_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['mim_created'])) {
			$data['mim_created'] = startup_env::get('timestamp');
		}

		if (empty($data['mim_updated'])) {
			$data['mim_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'mim_status' => self::STATUS_REMOVE,
			'mim_deleted' => startup_env::get('timestamp')
