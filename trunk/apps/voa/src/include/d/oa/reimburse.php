...
class voa_d_oa_reimburse extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.reimburse';
	/** 主键 */
	private static $__pk = 'rb_id';
	/** 所有字段名 */
	private static $__fields = array(
		'rb_id', 'm_uid', 'm_username', 'rb_subject', 'rb_type', 'rb_time', 'rbpc_id',
		'rb_status', 'rb_created', 'rb_updated', 'rb_deleted'
	);
	/** 审批中 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `rb_status`<'%d'
			ORDER BY `rb_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `rb_id`='%d' AND `rb_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `rb_id` IN (%n) AND `rb_status`<'%d'
			ORDER BY `rb_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND rb_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND rb_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['rb_status'])) {
			$data['rb_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['rb_created'])) {
			$data['rb_created'] = startup_env::get('timestamp');
		}

		if (empty($data['rb_updated'])) {
			$data['rb_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'rb_status' => self::STATUS_REMOVE,
			'rb_deleted' => startup_env::get('timestamp')
