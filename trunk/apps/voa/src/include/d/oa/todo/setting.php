...
class voa_d_oa_todo_setting extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.todo_setting';
	/** 主键 */
	private static $__pk = 'tds_key';
	/** 所有字段名 */
	private static $__fields = array(
		'tds_key', 'tds_value', 'tds_type', 'tds_comment',
		'tds_created', 'tds_updated', 'tds_deleted'
	);
	/** 正常 */
	...
	public static function fetch_by_pk($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `%i`='%d' AND `tds_status`<'%d'", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND tds_status<%d", array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['tds_status'])) {
			$data['tds_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['tds_created'])) {
			$data['tds_created'] = startup_env::get('timestamp');
		}

		if (empty($data['tds_updated'])) {
			$data['tds_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_pks($pks, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'tds_status' => self::STATUS_REMOVE,
			'tds_deleted' => startup_env::get('timestamp')
