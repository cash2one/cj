...
class voa_d_oa_todo_post extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.todo_post';
	/** 主键 */
	private static $__pk = 'tdp_id';
	/** 所有字段名 */
	private static $__fields = array(
		'tdp_id', 'm_uid', 'm_username', 'td_id', 'tdp_subject', 'tdp_message', 'tdp_first',
		'tdp_status', 'tdp_created', 'tdp_updated', 'tdp_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `tdp_status`<'%d'
			ORDER BY `tdp_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `tdp_id`='%d' AND `tdp_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `tdp_id` IN (%n) AND `tdp_status`<'%d'
			ORDER BY `tdp_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND tdp_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND tdp_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['tdp_status'])) {
			$data['tdp_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['tdp_created'])) {
			$data['tdp_created'] = startup_env::get('timestamp');
		}

		if (empty($data['tdp_updated'])) {
			$data['tdp_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'tdp_status' => self::STATUS_REMOVE,
			'tdp_deleted' => startup_env::get('timestamp')
		), db_help::field(self::$__pk, $ids), $unbuffered, false, $shard_key);
	}

	/**
	 * 删除指定td_id（单个或多个）的日报相关批注
