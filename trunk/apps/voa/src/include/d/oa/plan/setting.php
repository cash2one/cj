...
class voa_d_oa_plan_setting extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.plan_setting';
	/** 主键 */
	private static $__pk = 'pls_key';
	/** 所有字段名 */
	private static $__fields = array(
		'pls_key', 'pls_value', 'pls_type', 'pls_comment',
		'pls_status', 'pls_created', 'pls_updated', 'pls_deleted'
	);
	/** 正常 */
	...
	public static function fetch_by_pk($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `%i`='%d' AND `pls_status`<'%d'", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND pls_status<%d", array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['pls_status'])) {
			$data['pls_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['pls_created'])) {
			$data['pls_created'] = startup_env::get('timestamp');
		}

		if (empty($data['pls_updated'])) {
			$data['pls_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_pks($pks, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'pls_status' => self::STATUS_REMOVE,
			'pls_deleted' => startup_env::get('timestamp')
