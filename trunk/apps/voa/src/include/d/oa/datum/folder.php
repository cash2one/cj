...
class voa_d_oa_datum_folder extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.datum_folder';
	/** 主键 */
	private static $__pk = 'dtf_id';
	/** 所有字段名 */
	private static $__fields = array(
		'dtf_id', 'm_uid', 'm_username', 'dtf_name', 'dtf_displayorder',
		'dtf_status', 'dtf_created', 'dtf_updated', 'dtf_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `dtf_status`<'%d'
			ORDER BY `dtf_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `dtf_id`='%d' AND `dtf_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `dtf_id` IN (%n) AND `dtf_status`<'%d'
			ORDER BY `dtf_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND dtf_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND dtf_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['dtf_status'])) {
			$data['dtf_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['dtf_created'])) {
			$data['dtf_created'] = startup_env::get('timestamp');
		}

		if (empty($data['dtf_updated'])) {
			$data['dtf_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'dtf_status' => self::STATUS_REMOVE,
			'dtf_deleted' => startup_env::get('timestamp')
