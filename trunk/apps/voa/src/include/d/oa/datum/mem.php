...
class voa_d_oa_datum_mem extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.datum_mem';
	/** 主键 */
	private static $__pk = 'dtm_id';
	/** 所有字段名 */
	private static $__fields = array(
		'dtm_id', 'm_uid', 'm_username', 'dt_id',
		'dtm_status', 'dtm_created', 'dtm_updated', 'dtm_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `dtm_status`<'%d'
			ORDER BY `dtm_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `dtm_id`='%d' AND `dtm_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `dtm_id` IN (%n) AND `dtm_status`<'%d'
			ORDER BY `dtm_id` DESC", array(
	 ...
	 */
	public static function fetch_by_dt_id($dt_id, $start = 0, $limit = 0, $shard_key = array()) {
		return self::fetch_by_conditions(array('dt_id' => $dt_id), $start, $limit, $shard_key);
	}
	...
	public static function fetch_by_uid($uid, $start = 0, $limit = 0, $shard_key = array()) {
		$uid = intval($uid);
		return self::fetch_by_conditions(array(0, $uid), $start, $limit, $shard_key);
	}
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND dtm_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND dtm_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['dtm_status'])) {
			$data['dtm_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['dtm_created'])) {
			$data['dtm_created'] = startup_env::get('timestamp');
		}

		if (empty($data['dtm_updated'])) {
			$data['dtm_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'dtm_status' => self::STATUS_REMOVE,
			'dtm_deleted' => startup_env::get('timestamp')
