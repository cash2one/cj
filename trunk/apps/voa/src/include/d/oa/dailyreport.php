...
class voa_d_oa_dailyreport extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.dailyreport';
	/** 主键 */
	private static $__pk = 'dr_id';
	/** 所有字段名 */
	private static $__fields = array(
		'dr_id', 'm_uid', 'm_username', 'dr_subject',
		'dr_status', 'dr_created', 'dr_updated', 'dr_deleted','dr_type'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `dr_status`<'%d'
			ORDER BY `dr_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `dr_id`='%d' AND `dr_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `dr_id` IN (%n) AND `dr_status`<'%d'
			ORDER BY `dr_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND dr_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
	    return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND dr_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['dr_status'])) {
			$data['dr_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['dr_created'])) {
			$data['dr_created'] = startup_env::get('timestamp');
		}

		if (empty($data['dr_updated'])) {
			$data['dr_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'dr_status' => self::STATUS_REMOVE,
			'dr_deleted' => startup_env::get('timestamp')
	 ...
	 */
	public static function count_rank($start_time = 0, $end_time = 0, $shard_key = array()) {

		$list = array();
		$query = parent::_query(self::$__table
				, "SELECT `m_uid`, COUNT(`m_uid`) AS `_count` FROM %t
						WHERE `dr_reporttime`>%d AND `dr_reporttime`<%d
						AND `dr_status`<%d GROUP BY `m_uid` ORDER BY `_count` DESC, `m_uid` DESC"
				, array(self::$__table, $start_time, $end_time, self::STATUS_REMOVE), $shard_key);
		while ($row = parent::_fetch(self::$__table, $query)) {
			$list[$row['m_uid']] = $row['_count'];
		}
