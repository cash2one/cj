...
class voa_d_oa_todo extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.todo';
	/** 主键 */
	private static $__pk = 'td_id';
	/** 所有字段名 */
	private static $__fields = array(
		'td_id', 'm_uid', 'm_username', 'td_subject', 'td_calltime', 'td_exptime',
		'td_status', 'td_created', 'td_updated', 'td_deleted', 'td_completed', 'td_stared'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `td_status`<'%d'
			ORDER BY `td_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `td_id`='%d' AND `td_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `td_id` IN (%n) AND `td_status`<'%d'
			ORDER BY `td_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND td_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND td_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	 ...
	 * @return array              待办事项列表
	 */
	public static function fetch_by_conditions_and_order($conditions, $start = 0, $limit = 0, $orders = array(), $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND td_status<%d".db_help::orders($orders).db_help::limit($start, $limit), array(
				self::$__table, self::parse_conditions($conditions), self::STATUS_REMOVE, self::$__pk
			), self::$__pk, $shard_key
		);
	}
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['td_status'])) {
			$data['td_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['td_created'])) {
			$data['td_created'] = startup_env::get('timestamp');
		}

		if (empty($data['td_updated'])) {
			$data['td_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'td_status' => self::STATUS_REMOVE,
			'td_deleted' => startup_env::get('timestamp')
