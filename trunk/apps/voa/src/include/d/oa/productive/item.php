...
class voa_d_oa_productive_item extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.productive_item';
	/** 主键 */
	private static $__pk = 'pti_id';
	/** 所有字段名 */
	private static $__fields = array(
		'pti_id', 'pti_name', 'pti_describe', 'pti_parent_id', 'pti_score', 'pti_status', 'pti_created', 'pti_updated', 'pti_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `pti_status`<'%d'
			ORDER BY `pti_ordernum` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `pti_id`='%d' AND `pti_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `pti_id` IN (%n) AND `pti_status`<'%d'
			ORDER BY `pti_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND pti_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND pti_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['pti_status'])) {
			$data['pti_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['pti_created'])) {
			$data['pti_created'] = startup_env::get('timestamp');
		}

		if (empty($data['pti_updated'])) {
			$data['pti_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'pti_status' => self::STATUS_REMOVE,
			'pti_deleted' => startup_env::get('timestamp')
