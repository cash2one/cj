...
class voa_d_oa_productive_attachment extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.productive_attachment';
	/** 主键 */
	private static $__pk = 'ptat_id';
	/** 所有字段名 */
	private static $__fields = array(
		'ptat_id', 'm_uid', 'pt_id', 'at_id', 'ptsr_id',
		'ptat_status', 'ptat_created', 'ptat_updated', 'ptat_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `ptat_status`<'%d'
			ORDER BY `ptat_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `ptat_id`='%d' AND `ptat_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `ptat_id` IN (%n) AND `ptat_status`<'%d'
			ORDER BY `ptat_id` DESC", array(
	...
	public static function fetch_by_pt_id($pt_id, $start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT a.pt_id, a.ptsr_id, a.pti_id, a.ptat_id, b.* FROM %t AS a
			LEFT JOIN %t AS b ON a.at_id=b.at_id
			WHERE a.pt_id IN (%n) AND a.ptat_status<%d
			ORDER BY a.at_id DESC", array(
				self::$__table, voa_d_oa_common_attachment::$__table, (array)$pt_id, self::STATUS_REMOVE
			), self::$__pk, $shard_key
		);
	}
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND ptat_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND ptat_status<%d ORDER BY %i ASC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['ptat_status'])) {
			$data['ptat_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['ptat_created'])) {
			$data['ptat_created'] = startup_env::get('timestamp');
		}

		if (empty($data['ptat_updated'])) {
			$data['ptat_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'ptat_status' => self::STATUS_REMOVE,
			'ptat_deleted' => startup_env::get('timestamp')
	...
	public static function insert_multi($data, $shard_key = array()) {
		...
		foreach ($data as $_at) {
			$sql_ats[] = "(".implode(',', array(
				$_at['m_uid'], $_at['pt_id'], $_at['at_id'], $_at['pti_id'], $_at['ptsr_id'],
		...
		return parent::_query(self::$__table, "INSERT INTO %t(m_uid, pt_id, at_id, pti_id, ptsr_id, ptat_status, ptat_created, ptat_updated) VALUES".implode(',', $sql_ats), array(self::$__table), $shard_key);
	}
	...
	public static function fetch_by_pt_id_pti_id($pt_id, $pti_id, $start = 0, $limit = 0, $shard_key = array()) {
		...
		return parent::_fetch_all(self::$__table, "SELECT a.pt_id, a.ptsr_id, a.pti_id, a.ptat_id, b.* FROM %t AS a
			LEFT JOIN %t AS b ON a.at_id=b.at_id
			WHERE a.pt_id IN (%n) AND a.pti_id IN (%n) AND a.ptat_status<%d
			ORDER BY a.at_id DESC", array(
				self::$__table, voa_d_oa_common_attachment::$__table, (array)$pt_id, (array)$pti_id, self::STATUS_REMOVE
			), self::$__pk, $shard_key
		);
	}
}
