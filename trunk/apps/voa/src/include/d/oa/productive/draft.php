...
class voa_d_oa_productive_draft extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.productive_draft';
	/** 主键 */
	private static $__pk = 'ptd_id';
	/** 所有字段名 */
	private static $__fields = array(
		'ptd_id', 'm_openid', 'ptd_subject', 'ptd_message',
		'ptd_status', 'ptd_created', 'ptd_updated', 'ptd_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `ptd_status`<'%d'
			ORDER BY `ptd_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `ptd_id`='%d' AND `ptd_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `ptd_id` IN (%n) AND `ptd_status`<'%d'
			ORDER BY `ptd_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND ptd_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND ptd_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['ptd_status'])) {
			$data['ptd_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['ptd_created'])) {
			$data['ptd_created'] = startup_env::get('timestamp');
		}

		if (empty($data['ptd_updated'])) {
			$data['ptd_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'ptd_status' => self::STATUS_REMOVE,
			'ptd_deleted' => startup_env::get('timestamp')
	...
	public static function get_by_openid($openid, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `m_openid`=%s AND `ptd_status`<'%d' ORDER BY ptd_id DESC", array(
