...
class voa_d_oa_productive_setting extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.productive_setting';
	/** 主键 */
	private static $__pk = 'pts_key';
	/** 所有字段名 */
	private static $__fields = array(
		'pts_key', 'pts_value', 'pts_type', 'pts_comment',
		'pts_created', 'pts_updated', 'pts_deleted'
	);
	/** 正常 */
	...
	public static function fetch_by_pk($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `%i`='%d' AND `pts_status`<'%d'", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND pts_status<%d", array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['pts_status'])) {
			$data['pts_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['pts_created'])) {
			$data['pts_created'] = startup_env::get('timestamp');
		}

		if (empty($data['pts_updated'])) {
			$data['pts_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_pks($pks, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'pts_status' => self::STATUS_REMOVE,
			'pts_deleted' => startup_env::get('timestamp')
