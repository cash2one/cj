<?php
/**
 * voa_d_oa_qymsg_media
 * 消息素材表
 * Create By Deepseath
 * $Author$
 * $Id$
 */
class voa_d_oa_qymsg_media extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.qymsg_media';
	/** 主键 */
	private static $__pk = 'qymm_id';
	/** 字段前缀 */
	private static $__prefix = 'qymm_';
	/** 正常 */
	...
	public static function search_by_conditions($conditions = array()) {
		$where = array();
		if (isset($conditions['qymm_type']) && $conditions['qymm_type']) {
			$where[] = db_help::field('qymm_type', $conditions['qymm_type']);
		}

		if (isset($conditions['qymm_status']) && $conditions['qymm_status']) {
			$where[] = db_help::field('qymm_status', $conditions['qymm_status']);
		} else {
			$where[] = db_help::field('qymm_status', self::STATUS_REMOVE, '<');
		}

		if (isset($conditions['ca_id']) && $conditions['ca_id']) {
			$where[] = db_help::field('ca_id', $conditions['ca_id']);
		}

		if (isset($conditions['qymm_title']) && $conditions['qymm_title'] != '') {
			$where[] = db_help::field('qymm_title', '%'.addcslashes($conditions['qymm_title'], '%_').'%', 'like');
		}
	...
	public static function fetch_all_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array) parent::_fetch_all(self::$__table, "SELECT * FROM %t WHERE %i ORDER BY `%i` DESC %i", array(
			self::$__table, self::search_by_conditions($conditions), self::$__pk, db_help::limit($start, $limit)
		), self::$__pk, $shard_key);
	}
