...
class voa_d_oa_vnote extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.vnote';
	/** 主键 */
	private static $__pk = 'vn_id';
	/** 所有字段名 */
	private static $__fields = array(
		'vn_id', 'm_uid', 'm_username', 'vn_subject',
		'vn_status', 'vn_created', 'vn_updated', 'vn_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `vn_status`<'%d'
			ORDER BY `vn_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `vn_id`='%d' AND `vn_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `vn_id` IN (%n) AND `vn_status`<'%d'
			ORDER BY `vn_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND vn_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND vn_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['vn_status'])) {
			$data['vn_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['vn_created'])) {
			$data['vn_created'] = startup_env::get('timestamp');
		}

		if (empty($data['vn_updated'])) {
			$data['vn_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'vn_status' => self::STATUS_REMOVE,
			'vn_deleted' => startup_env::get('timestamp')
