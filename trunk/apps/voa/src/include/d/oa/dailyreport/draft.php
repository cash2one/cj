...
class voa_d_oa_dailyreport_draft extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.dailyreport_draft';
	/** 主键 */
	private static $__pk = 'drd_id';
	/** 所有字段名 */
	private static $__fields = array(
		'drd_id', 'm_openid', 'drd_subject', 'drd_message',
		'drd_status', 'drd_created', 'drd_updated', 'drd_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `drd_status`<'%d'
			ORDER BY `drd_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `drd_id`='%d' AND `drd_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `drd_id` IN (%n) AND `drd_status`<'%d'
			ORDER BY `drd_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND drd_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND drd_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['drd_status'])) {
			$data['drd_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['drd_created'])) {
			$data['drd_created'] = startup_env::get('timestamp');
		}

		if (empty($data['drd_updated'])) {
			$data['drd_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'drd_status' => self::STATUS_REMOVE,
			'drd_deleted' => startup_env::get('timestamp')
		), db_help::field(self::$__pk, $ids), $unbuffered, false, $shard_key);
	}

	/** 根据 id 值, 读取数据 */
	public static function get_by_openid($openid, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `m_openid`=%s AND `drd_status`<'%d' ORDER BY drd_id DESC", array(
