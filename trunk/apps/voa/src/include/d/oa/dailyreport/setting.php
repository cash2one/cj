...
class voa_d_oa_dailyreport_setting extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.dailyreport_setting';
	/** 主键 */
	private static $__pk = 'drs_key';
	/** 所有字段名 */
	private static $__fields = array(
		'drs_key', 'drs_value', 'drs_type', 'drs_comment',
		'drs_created', 'drs_updated', 'drs_deleted'
	);
	/** 正常 */
	...
	public static function fetch_by_pk($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `%i`='%d' AND `drs_status`<'%d'", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND drs_status<%d", array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['drs_status'])) {
			$data['drs_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['drs_created'])) {
			$data['drs_created'] = startup_env::get('timestamp');
		}

		if (empty($data['drs_updated'])) {
			$data['drs_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_pks($pks, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'drs_status' => self::STATUS_REMOVE,
			'drs_deleted' => startup_env::get('timestamp')
