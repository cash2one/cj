...
class voa_d_oa_dailyreport_post extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.dailyreport_post';
	/** 主键 */
	private static $__pk = 'drp_id';
	/** 所有字段名 */
	private static $__fields = array(
		'drp_id', 'm_uid', 'm_username', 'dr_id', 'drp_subject', 'drp_message', 'drp_first',
		'drp_status', 'drp_created', 'drp_updated', 'drp_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `drp_status`<'%d'
			ORDER BY `drp_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `drp_id`='%d' AND `drp_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `drp_id` IN (%n) AND `drp_status`<'%d'
			ORDER BY `drp_id` DESC", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND drp_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert(&$data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['drp_status'])) {
			$data['drp_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['drp_created'])) {
			$data['drp_created'] = startup_env::get('timestamp');
		}

		if (empty($data['drp_updated'])) {
			$data['drp_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'drp_status' => self::STATUS_REMOVE,
			'drp_deleted' => startup_env::get('timestamp')
		), db_help::field(self::$__pk, $ids), $unbuffered, false, $shard_key);
	}

	/**
	 * 删除指定dr_id（单个或多个）的日报相关批注
