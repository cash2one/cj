...
class voa_d_oa_remind extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.remind';
	/** 主键 */
	private static $__pk = 'rm_id';
	/** 所有字段名 */
	private static $__fields = array(
		'rm_id', 'm_uid', 'm_username', 'rm_subject', 'rm_message', 'rm_remindts',
		'rm_status', 'rm_created', 'rm_updated', 'rm_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `rm_status`<'%d'
			ORDER BY `rm_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `rm_id`='%d' AND `rm_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `rm_id` IN (%n) AND `rm_status`<'%d'
			ORDER BY `rm_id` DESC", array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['rm_status'])) {
			$data['rm_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['rm_created'])) {
			$data['rm_created'] = startup_env::get('timestamp');
		}

		if (empty($data['rm_updated'])) {
			$data['rm_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'rm_status' => self::STATUS_REMOVE,
			'rm_deleted' => startup_env::get('timestamp')
