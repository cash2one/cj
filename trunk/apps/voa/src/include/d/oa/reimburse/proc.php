...
class voa_d_oa_reimburse_proc extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.reimburse_proc';
	/** 主键 */
	private static $__pk = 'rbpc_id';
	/** 所有字段名 */
	private static $__fields = array(
		'rbpc_id', 'rb_id', 'm_uid', 'm_username', 'rbpc_remark',
		'rbpc_status', 'rbpc_created', 'rbpc_updated', 'rbpc_deleted'
	);
	/** 审批中 */
	const STATUS_NORMAL = 1;
	/** 通过并转审批 */
	const STATUS_TRANSMIT = 2;
	/** 已通过 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `rbpc_status`<'%d'
			ORDER BY `rbpc_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `rbpc_id`='%d' AND `rbpc_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `rbpc_id` IN (%n) AND `rbpc_status`<'%d'
			ORDER BY `rbpc_id` DESC", array(
	...
	public static function fetch_by_rb_id($rb_id, $start = 0, $limit = 0, $shard_key = array()) {
		return self::fetch_by_conditions(array('rb_id' => $rb_id), $start, $limit, $shard_key);
	}

	/**
	 * 根据 uid, updated 读取我发起的报销列表
	 * @param int $uid
	 * @param int $update
	 * @param number $start
	 * @param number $limit
	 ...
	 */
	public static function fetch_mine($uid, $update, $start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT b.* FROM %t AS a
			LEFT JOIN %t AS b ON a.rb_id=b.rb_id
			WHERE a.m_uid=b.m_uid AND a.m_uid=%d AND a.rbpc_updated<%d AND `rbpc_status`<'%d'
			ORDER BY a.rbpc_id DESC".db_help::limit($start, $limit), array(
				self::$__table, voa_d_oa_reimburse::$__table, $uid, $update, self::STATUS_REMOVE
			), self::$__pk, $shard_key
		);
	}

	/**
	 * 根据 uid, updated 读取我审批的报销列表
	 * @param int $uid
	 * @param int $update
	 * @param number $start
	 * @param number $limit
	 * @throws service_exception
	 */
	public static function fetch_deal($uid, $update, $status = array(), $start = 0, $limit = 0, $shard_key = array()) {
		$wheres = array('a.m_uid!=b.m_uid AND a.m_uid=%d AND a.rbpc_updated<%d');
		$params = array($uid, $update);
		/** 如果指定了读取哪些状态 */
		if (!empty($status)) {
			$params[] = $status;
			$wheres[] = 'a.rbpc_status IN (%n)';
		} else {
			$params[] = self::STATUS_REMOVE;
			$wheres[] = 'a.rbpc_status<%d';
		}

		$wherestr = implode(' AND ', $wheres);
		array_unshift($params, self::$__table, voa_d_oa_reimburse::$__table);

		return parent::_fetch_all(self::$__table, "SELECT b.* FROM %t AS a
			LEFT JOIN %t AS b ON a.rb_id=b.rb_id
			WHERE {$wherestr}
			ORDER BY a.rbpc_id DESC".db_help::limit($start, $limit), $params, self::$__pk, $shard_key
		);
	}
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND rbpc_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND rbpc_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['rbpc_status'])) {
			$data['rbpc_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['rbpc_created'])) {
			$data['rbpc_created'] = startup_env::get('timestamp');
		}

		if (empty($data['rbpc_updated'])) {
			$data['rbpc_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'rbpc_status' => self::STATUS_REMOVE,
			'rbpc_deleted' => startup_env::get('timestamp')
