...
class voa_d_oa_reimburse_post extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.reimburse_post';
	/** 主键 */
	private static $__pk = 'rbpt_id';
	/** 所有字段名 */
	private static $__fields = array(
		'rbpt_id', 'rb_id', 'm_uid', 'm_username', 'rbpt_subject', 'rbpt_message', 'rbpt_first',
		'rbpt_status', 'rbpt_created', 'rbpt_updated', 'rbpt_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `rbpt_status`<'%d'
			ORDER BY `rbpt_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `rbpt_id`='%d' AND `rbpt_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `rbpt_id` IN (%n) AND `rbpt_status`<'%d'
			ORDER BY `rbpt_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND rbpt_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND rbpt_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['rbpt_status'])) {
			$data['rbpt_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['rbpt_created'])) {
			$data['rbpt_created'] = startup_env::get('timestamp');
		}

		if (empty($data['rbpt_updated'])) {
			$data['rbpt_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'rbpt_status' => self::STATUS_REMOVE,
			'rbpt_deleted' => startup_env::get('timestamp')
