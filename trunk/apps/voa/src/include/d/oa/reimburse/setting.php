...
class voa_d_oa_reimburse_setting extends dao_mysql {
	/** 表名 */
	public static $__table = 'oa.reimburse_setting';
	/** 主键 */
	private static $__pk = 'rbs_key';
	/** 所有字段名 */
	private static $__fields = array(
		'rbs_key', 'rbs_value', 'rbs_type', 'rbs_comment',
		'rbs_created', 'rbs_updated', 'rbs_deleted'
	);
	/** 正常 */
	...
	public static function fetch_by_pk($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `%i`='%d' AND `rbs_status`<'%d'", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND rbs_status<%d", array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['rbs_status'])) {
			$data['rbs_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['rbs_created'])) {
			$data['rbs_created'] = startup_env::get('timestamp');
		}

		if (empty($data['rbs_updated'])) {
			$data['rbs_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_pks($pks, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'rbs_status' => self::STATUS_REMOVE,
			'rbs_deleted' => startup_env::get('timestamp')
