<?php
/**
 * voa_d_oa_reimburse_bill_attachment
 * 应用.报销/清单/附件表
 * Create By Deepseath
 * $Author$
 * $Id$
 */
class voa_d_oa_reimburse_bill_attachment extends dao_mysql {

	/** 表名 */
	public static $__table = 'oa.reimburse_bill_attachment';
	/** 主键 */
	private static $__pk = 'rbbat_id';
	/** 字段前缀 */
	private static $__prefix = 'rbbat_';
	/** 所有字段名 */
	private static $__fields = array(
		'rbbat_id', 'rbb_id', 'at_id',
		'm_id', 'm_username',
		'rbbat_status', 'rbbat_created', 'rbbat_upatated', 'rbbat_deleted'
	);
	 ...
	 * @return array
	 */
	public static function fetch_all_by_rbb_ids($rbb_ids, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t WHERE %i AND %i ORDER BY `rbbat_id` ASC", array(
			self::$__table, db_help::field('rbb_id', $rbb_ids), db_help::field('rbbat_status', self::STATUS_REMOVE, '<')
		), self::$__pk, $shard_key);
	}
