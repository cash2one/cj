...
class voa_d_cyadmin_common_adminergroup extends dao_mysql {
	/** 表名 */
	public static $__table = 'cyadmin.common_adminergroup';
	/** 主键 */
	private static $__pk = 'cag_id';
	/** 字段前缀 */
	private static $__prefix = 'cag_';
	/** 正常 */
	 ...
	 * (D) 统计某个名字的管理组个数
	 * @author Deepseath
	 * @param string $title 管理组名称
	 * @param number $cag_id 除此ID外
	 * @return number
	 */
	public static function count_by_title_notid($title, $cag_id = 0, $shard_key = array()) {
		return parent::_result_first(self::$__table,
			"SELECT COUNT(`cag_title`) FROM %t WHERE `cag_title`=%s AND `cag_status`<%d AND `cag_id`<>%d",
			array(self::$__table, $title, self::STATUS_REMOVE, $cag_id), $shard_key
		);
	}
