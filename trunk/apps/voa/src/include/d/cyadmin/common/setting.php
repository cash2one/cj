...
class voa_d_cyadmin_common_setting extends dao_mysql {
	/** 表名 */
	public static $__table = 'cyadmin.common_setting';
	/** 主键 */
	...
	public static function fetch($key, $shard_key = array()) {
		$data = self::fetch_by_key(array($key), $shard_key);
	...
	public static function fetch_by_keys($keys, $shard_key = array()) {
		return parent::_fetch_all(self::$__table,
			"SELECT * FROM %t WHERE cs_name IN (%n) AND cs_status<'%d'",
