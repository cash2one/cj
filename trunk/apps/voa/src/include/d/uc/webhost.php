 ...
 * $Id$
 */
class voa_d_uc_webhost extends dao_mysql {
	/** 表名 */
	public static $__table = 'uc.webhost';
	/** 主键 */
	private static $__pk = 'web_id';
	/** 字段前缀 */
	private static $__prefix = 'web_';
	/** 正常 */
	...
	public static function fetch_by_maximum($shard_key = array()) {
		return (array)parent::_fetch_first(self::$__table, "SELECT * FROM %t WHERE `web_count`<`web_maximum` ORDER BY `web_count` DESC LIMIT 1", array(
			self::$__table
		), $shard_key);
	}
	...
	public static function fetch_by_minimum($shard_key = array()) {
		return (array)parent::_fetch_first(self::$__table, "SELECT * FROM %t ORDER BY `web_count` ASC LIMIT 1", array(self::$__table), $shard_key);
	}
	 ...
	 * @return boolean
	 */
	public static function count_add_by_web_id($web_id, $unit = 1, $shard_key = array()) {
		return parent::_incr(self::$__table, 'web_count', db_help::field('web_id', $web_id), array(), $unit);
	}
