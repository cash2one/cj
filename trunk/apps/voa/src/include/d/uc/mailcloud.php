...
class voa_d_uc_mailcloud extends dao_mysql {
	/** 表名 */
	public static $__table = 'uc.mailcloud';
	/** 主键 */
	private static $__pk = 'mc_id';
	/** 所有字段名 */
	private static $__fields = array(
		'mc_id', 'mc_email', 'mc_tplname', 'mc_subject', 'mc_vars', 'mc_repeat',
		'mc_status', 'mc_created', 'mc_updated', 'mc_deleted'
	);
	/** 已发送 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `mc_status`<'%d'
			ORDER BY `mc_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `mc_id`='%d' AND `mc_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `mc_id` IN (%n) AND `mc_status`<'%d'
			ORDER BY `mc_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND mc_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND mc_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['mc_status'])) {
			$data['mc_status'] = self::STATUS_BANDED;
		}

		if (empty($data['mc_created'])) {
			$data['mc_created'] = startup_env::get('timestamp');
		}

		if (empty($data['mc_updated'])) {
			$data['mc_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'mc_status' => self::STATUS_REMOVE,
			'mc_deleted' => startup_env::get('timestamp')
	...
	public static function fetch_by_email($email, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t WHERE mc_email=%s AND mc_status<%d", array(
	...
	public static function insert_multi($data, $shard_key = array()) {
		...
		foreach ($data as $_m) {
			$arr[] = "(".implode(',', array(
				db_help::quote($_m['mc_tplname']),
				db_help::quote($_m['mc_email']),
				db_help::quote($_m['mc_subject']),
				db_help::quote($_m['mc_vars']),
				db_help::quote($_m['mc_repeat']),
				db_help::quote($_m['mc_status'])
			)).")";
		}
		...
		return parent::_query(self::$__table, "INSERT INTO %t(mc_tplname, mc_email, mc_subject, mc_vars, mc_repeat, mc_status) VALUES%i", array(self::$__table, implode(',', $arr)), $shard_key);
	}
}
