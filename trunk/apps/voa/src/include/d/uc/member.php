 ...
 * $Id$
 */
class voa_d_uc_member extends dao_mysql {
	/** 表名 */
	public static $__table = 'uc.member';
	/** 主键 */
	private static $__pk = 'm_id';
	/** 字段前缀 */
	 ...
	 * @return number
	 */
	public static function count_by_mobilephone_not_id($mobilephone, $m_id = 0, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(`m_mobilephone`) FROM %t WHERE %i AND %i AND %i", array(
			db_help::field('m_mobilephone', $mobilephone),
			$m_id ? db_help::field('m_id', $m_id, '<>') : 1,
			db_help::field('m_status', self::STATUS_REMOVE, '<')
		));
	}
	 ...
	 * @return number
	 */
	public static function count_by_email_not_id($email, $m_id = 0, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(`m_email`) FROM %t WHERE %i AND %i AND %i", array(
			db_help::field('m_email', $email),
			$m_id ? db_help::field('m_id', $m_id, '<>') : 1,
			db_help::field('m_status', self::STATUS_REMOVE, '<')
		));
	}
