...
class voa_d_uc_enterprise_profile extends dao_mysql {
	/** 表名 */
	public static $__table = 'uc.enterprise_profile';
	/** 主键 */
	private static $__pk = 'ep_id';
	/** 字段前缀 */
	private static $__prefix = 'epp_';
	/** 正常 */
	...
	public static function count_by_field_not_in_ep_id($field, $value, $not_in_ep_id = 0, $shard_key = array()) {
		return (int)parent::_count(self::$__table, "SELECT COUNT(%i) FROM %t WHERE %i AND %i", array(
			$field, self::$__table, db_help::field($field, $value),
			$not_in_ep_id ? db_help::field('ep_id', $not_in_ep_id, '<>') : 1
		), $shard_key);
	}
