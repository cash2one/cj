...
class voa_d_uc_suggestion extends dao_mysql {
	/** 表名 */
	public static $__table = 'uc.suggestion';
	/** 主键 */
	private static $__pk = 'sug_id';
	/** 所有字段名 */
	private static $__fields = array(
		'sug_id', 'sug_message', 'sug_domain', 'sug_username',
		'sug_status', 'sug_created', 'sug_updated', 'sug_deleted'
	);
	/** 正常 */
	...
	public static function fetch_all($start = 0, $limit = 0, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `sug_status`<'%d'
			ORDER BY `sug_id` DESC".db_help::limit($start, $limit), array(
	...
	public static function fetch_by_id($id, $shard_key = array()) {
		return parent::_fetch_first(self::$__table, "SELECT * FROM %t
			WHERE `sug_id`='%d' AND `sug_status`<'%d'", array(
	...
	public static function fetch_by_ids($ids, $shard_key = array()) {
		return parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE `sug_id` IN (%n) AND `sug_status`<'%d'
			ORDER BY `sug_id` DESC", array(
	...
	public static function count_by_conditions($conditions, $shard_key = array()) {
		return (int)parent::_result_first(self::$__table, "SELECT COUNT(*) FROM %t WHERE %i AND sug_status<%d", array(
	...
	public static function fetch_by_conditions($conditions, $start = 0, $limit = 0, $shard_key = array()) {
		return (array)parent::_fetch_all(self::$__table, "SELECT * FROM %t
			WHERE %i AND sug_status<%d ORDER BY %i DESC".db_help::limit($start, $limit), array(
	...
	public static function insert($data, $return_insert_id = false, $replace = false, $silent = false, $shard_key = array()) {
		if (empty($data['sug_status'])) {
			$data['sug_status'] = self::STATUS_NORMAL;
		}

		if (empty($data['sug_created'])) {
			$data['sug_created'] = startup_env::get('timestamp');
		}

		if (empty($data['sug_updated'])) {
			$data['sug_updated'] = startup_env::get('timestamp');
		}
	...
	public static function delete_by_ids($ids, $unbuffered = false, $shard_key = array()) {
		return parent::_update(self::$__table, array(
			'sug_status' => self::STATUS_REMOVE,
			'sug_deleted' => startup_env::get('timestamp')
