...
class voa_server_base extends rpc_server {
	...
	protected function _execute($class_name, $method, $args) {
		...
		try {
			...
			if (!$clazz || !in_array($method, $methods)) {
				$e = new rpc_exception('unknown method 3', 502);
			...
			if ($cache && $cache['ttl'] > 0) {
				...
				try {
					$serv =& service::factory('voa_s_oa_cache_open', array('pluginid' => startup_env::get('pluginid')));
			...
			logger::error($e);
			$e = new rpc_exception('An unknown error occurred. Please resubmit the request. ', 2);
			return $this->_output_error($e);
		}
