 ...
 * Time: 上午10:23
 */
class voa_c_admincp_api_member_list extends voa_c_admincp_api_member_base {
    ...
    private function __list() {
        ...
        if ($cd_id < 1) {
            $this->_output_result(array('list' => array()));
            return;
        }
        ...
        $members = $serv_m->fetch_all_by_conditions($condi, array('m_displayorder' => 'DESC'), $start, $limit);
        ...
        $this->_output_result(array('list' => $members, 'pages' => $pages));
    }
    ...
    private function __search($kw) {
        ...
        /** 根据用户名搜索通讯录信息 */
        $serv_ms = &service::factory('voa_s_oa_member_search', array('pluginid' => 0));
        $conditions = array(
            'ms_message' => array('%'.$kw.'%', 'like')
        );
        /** 从搜索表取通讯录数据 */
        $list = $serv_ms->fetch_by_conditions($conditions);
        if ($list) {
