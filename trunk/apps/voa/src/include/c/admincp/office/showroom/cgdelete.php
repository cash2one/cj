<?php
/**
 * voa_c_admincp_office_showroom_atdelete
 * 企业后台/微办公管理/陈列/删除目录
 * Create By YanWenzhong
 * $Author$
 * $Id$
 */
class voa_c_admincp_office_showroom_cgdelete extends voa_c_admincp_office_showroom_base {
	...
	public function execute() {
		...
		$uda_categorydelete = &uda::factory('voa_uda_frontend_showroom_action_categorydelete');

		//如果要删除的目录中有文章，则不让删除
		$uda_articlelist = &uda::factory('voa_uda_frontend_showroom_action_articlelist');
		foreach( $ids as $id ) {
			$count = $uda_articlelist->count_articles_by_category_id($id);
			if ($count > 0) { //只要其中一个目录 下有文章，则不让删除
				$this->message('error', voa_errcode_oa_showroom::DELETE_CATEGORY_FAILED1);
				return;
			}
		}
