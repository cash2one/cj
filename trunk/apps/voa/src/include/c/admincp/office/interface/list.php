...
/**
 * voa_c_admincp_office_interface_list
 * 企业后台/测试应用/接口列表
 * Create By gaosong
 * $Author$
 * $Id$
 */
class voa_c_admincp_office_interface_list extends voa_c_admincp_office_interface_base {
	...
	public function execute() {
		...
		// 接口编辑url
		$this->view->set('edit_url', $this->cpurl($this->_module, $this->_operation, 'edit', $this->_module_plugin_id, array('n_id' => '')));
		// 删除接口url
		$this->view->set('delete_url', $this->cpurl($this->_module, $this->_operation, 'delete', $this->_module_plugin_id, array('n_id' => '')));

		// 输出模板
		$this->output('office/interface/list');
	}
