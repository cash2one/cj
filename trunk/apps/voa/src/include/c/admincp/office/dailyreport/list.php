 ...
 * $Id$
 */
class voa_c_admincp_office_dailyreport_list extends voa_c_admincp_office_dailyreport_base {
	...
	public function execute() {
		...
		$searchDefaults = array(
			'm_uid' => 0,
			'cab_realname_author' => '', // 报告人姓名
			'cab_realname_receive' => '', // 接收人姓名
			'dr_subject' => '', // 报告主题
			'begintime' => '', // 报告提交开始时间
			'endtime' => '', // 报告提交结束时间
			'dr_type' => ''
...
;
		...
		list($total, $multi, $searchBy, $list) = $this->_search_dailyreport($this->_module_plugin_id, $issearch, $searchDefaults, $perpage);
		// 请求的是导出操作
		if ($this->request->get('is_dump')) {
		...
		$this->view->set('searchBy', $searchBy);
		$this->view->set('dailyType', $p_sets['daily_type']);
		...
		$this->view->set('deleteUrlBase', $this->cpurl($this->_module, $this->_operation, 'delete', $this->_module_plugin_id, array(
			'dr_id' => ''
		)));
		...
		$this->output('office/dailyreport/dailyreport_list');
	}
