 ...
 * $Id$
 */
class voa_c_admincp_office_productive_rating extends voa_c_admincp_office_base {
	...
	protected function _search_namecard($cp_pluginid, $issearch, $searchDefault = array(), $perpage = 10) {
		...
		$list = array();
		$db = &service::factory('voa_s_oa_namecard');
		$total = $db->count_by_conditions($conditions);
		$multi = '';
		if ($total > 0) {
			$pagerOptions = array(
					'total_items' => $total,
			...
			$tmp = $db->fetch_all_by_conditions($conditions, $pagerOptions['start'], $pagerOptions['per_page']);
			$m_uids = array();
			$ncj_ids = array();
			$ncc_ids = array();
			$nccf_ids = array();
