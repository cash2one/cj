<?php
/**
 * voa_c_admincp_setting_askoff_modify
 * 企业后台/系统设置/请假审批/更改设置
 * Create By Deepseath
 * $Author$
 * $Id$
 */
class voa_c_admincp_office_askoff_setting extends voa_c_admincp_setting_base {
	...
	public function execute() {
		...
		/** 当前操作的数据表 */
		$this->_current_operation_table = 'askoff';
		...
		$this->_current_keys_setting = array(
			...
			'types' => array(
				...
				'id' => 'types',
				'name' => 'types',
				'comment' => '设置请假的类型，显示顺序按正序排序、类型名称不能超过 '.$this->_name_max_length.' 个字不允许特殊字符，最多允许创建 '.$this->_types_max_count.' 个请假类型，字符不符或超出类型数限制则会忽略。类型编号与类型名称是系统自动对应关系，一旦确定不可更改',
				'title' => '请假类型设置',
				'custom' => array(
			...
			'upload_image' => array(
				...
				'id' => 'upload_image',
				'name' => 'upload_image',
				'comment' => '选择“是”则新建请假时允许上传图片，否则不允许。',
				'title' => '是否允许上传请假相关图片'
			),
		...
		if ($this->_is_post()) {
			...
			if (empty($types_name)) {
				$this->message('error', '请正确设置请假类型');
			}

			if (count($types_name) > $this->_types_max_count) {
				$this->message('error', '系统最多允许添加 '.$this->_types_max_count.' 个请假类型，请减少后再试');
			}

			//TODO ...这里将类型名转为GBK进行排序..也许不需要这么严格...
			array_multisort($types_displayorder, SORT_ASC, SORT_NUMERIC, riconv($types_name, 'utf-8', 'gbk'), SORT_ASC, SORT_REGULAR, $types_tmp);
