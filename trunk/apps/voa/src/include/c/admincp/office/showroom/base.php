 ...
 * $Id$
 */
class voa_c_admincp_office_showroom_base extends voa_c_admincp_office_base {
	...
	protected function _before_action($action) {
		...
		//FIXME ！！！涉及指定应用更新问题
		$this->_p_sets = voa_h_cache::get_instance()->get('plugin.showroom.setting', 'oa');
	...
	protected function _article_edit($ta_id = 0, $is_new = false) {
		...
		$uda = &uda::factory('voa_uda_frontend_showroom_action_articleedit');
		$uda_view = &uda::factory('voa_uda_frontend_showroom_action_articleview');
		$article = array();
		...
		if ($this->_is_post()) {
			$uda_validator = &uda::factory('voa_uda_frontend_showroom_validator');
			...
			if ($this->request->get('send')) { //发送消息提醒
				...
				$uda_notice = &uda::factory('voa_uda_frontend_showroom_wxqynotice');
		...
		// 取得目录列表
		$uda_categorylist = &uda::factory('voa_uda_frontend_showroom_action_categorylist');
		...
		$this->output('office/showroom/article_form');
	...
	protected function _category_edit($tc_id = 0, $is_new = false) {
		...
		$this->output('office/showroom/category_form');
	...
	private function __get_right_limit ($tc_id) {
		$uda_category = &uda::factory('voa_uda_frontend_showroom_action_categoryedit');
		$category = array();
		$existed_range = array();
