 ...
 * $Id$
 */
class voa_c_frontend_train_detail extends voa_c_frontend_train_base {
	...
	public function execute() {
		...
		try{
			...
			if (!$article) { //判断文章是否存在
				voa_h_func::throw_errmsg(voa_errcode_oa_train::ARTICLE_NOT_EXIST);
			}
			//判断有无阅读文章权限
			$department_ids = $uda->get_department_id($m_uid);
			if ($article['is_all'] != voa_d_oa_train_articleright::IS_ALL
					&& !in_array($m_uid, $article['contacts']) && !array_intersect($department_ids, $article['deps'])
			) {
				voa_h_func::throw_errmsg(voa_errcode_oa_train::ARTICLE_NO_RIGHT);
			}
		...
		// 引入应用模板
		$this->_output('mobile/train/view');
	}
