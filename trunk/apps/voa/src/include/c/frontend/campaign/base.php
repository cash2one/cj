...
class voa_c_frontend_campaign_base extends voa_c_frontend_base {
	 ...
	 * 获取会议配置信息
	 */
	public static function fetch_cache_campaign_setting() {

		$d = new voa_d_oa_campaign_setting();
		$data = $d->list_all();
		$arr = array();
		$_pluginid = - 1;
		foreach ($data as $v) {
		...
		self::_check_agentid($arr, 'campaign');
		return $arr;
	}
}
