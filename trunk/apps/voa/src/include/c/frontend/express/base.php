 ...
 * $Id$
 */
class voa_c_frontend_express_base extends voa_c_frontend_base
{
	...
	protected function _before_action($action)
	{
		...
		$this->view->set('css_file', 'app_express.css');
		return true;
	}
	...
	{
	    //获取同事社区设置信息
		$this->_p_sets = voa_h_cache::get_instance()->get('plugin.express.setting', 'oa');
	 ...
	 * 获取快递助手配置
	 */
	public static function fetch_cache_express_setting() {

		$serv = &service::factory('voa_s_oa_express_setting');
		$data = $serv->list_all();
		$arr = array();
		$_pluginid = - 1;
		foreach ($data as $v) {
		...
		self::_check_agentid($arr, 'express');
		return $arr;
	}
}
