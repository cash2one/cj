...
class voa_c_frontend_travel_base extends voa_c_frontend_base {
	...
	protected function _before_action($action) {
		...
		$this->session->set('logintype', 'qy');
		//$this->view->set('navtitle', '产品');
		$this->view->set('travel_set', $this->_p_sets);

		$class = voa_h_cache::get_instance()->get('plugin.travel.goodsclass', 'oa');
		$this->view->set('goodsclass', $class);
		$this->view->set('jsongoodsclass', rjson_encode($class));
	...
	public function get_tpl_style() {
		...
		$style = $plugin_identifier;
		if (!empty($this->_p_sets['goods_tpl_style'])) {
			$style = $this->_p_sets['goods_tpl_style'];
		}

		// 将应用唯一标识名注入模板变量
		$this->view->set('plugin_identifier', $style);
		return $style;
	}
	...
	public static function fetch_cache_travel_setting() {
		...
		self::_check_agentid($arr, 'travel');
		return $arr;
	}
	...
	public static function fetch_goods_tablecol($tid) {
