...
class voa_c_frontend_askoff_reply extends voa_c_frontend_askoff_base {
	...
	public function execute() {
		...
		/** 获取评论信息 */
		$servcmt = &service::factory('voa_s_oa_askoff_comment', array('pluginid' => startup_env::get('pluginid')));
		$cmt = $servcmt->fetch_by_id($afc_id);
		if (empty($afc_id) || empty($cmt)) {
			$this->_error_message('askoff_comment_not_exist', get_referer());
		}
		...
		/** 回复信息入库 */
		$servrpy = &service::factory('voa_s_oa_askoff_reply', array('pluginid' => startup_env::get('pluginid')));
		...
		/** 提示操作成功 */
		$this->_success_message('回复操作成功', "/askoff/view/{$cmt['af_id']}");
	}
