<?php
/**
 * voa_c_api_askfor_post_approve
 * 通过审批
 * $Author$
 * $Id$
 */

class voa_c_api_askfor_post_approve extends voa_c_api_askfor_verify {
	...
	public function execute() {
		...
		if (empty($this->_params['af_id'])) {
			return $this->_set_errcode(voa_errcode_api_askfor::AF_ID_NULL);
		}
		/*审批进度检查*/
		...
		if (!$uda->askfor_approve($this->_askfor, $this->_proc)) {
		...
		$content = "审批状态: {$this->_member['m_username']} 已同意\n"
				 . "------------------\n"
