...
class voa_c_api_common_get_wxqymenu extends voa_c_api_common_abstract {
	...
	public function execute() {
		...
		if (!$this->__is_open) {
			$this->_set_errcode('100:获取微信企业号自定义菜单接口未开启');
			return false;
		}
		...
		if (!$qywx_menu->get($this->__plugin['cp_agentid'], $this->_result)) {
			$error = empty($qywx_menu->menu_error) ? '获取应用菜单发生未知错误' : $qywx_menu->menu_error;
