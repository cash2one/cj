<?php
/**
 * voa_c_api_reimburse_base
 * 报销基础控制器
 * $Author$
 * $Id$
 */
class voa_c_api_reimburse_base extends voa_c_api_base {
	...
	public static function wx_attach($attachment, $data, $plugin) {
		...
		/** 信息入总库 */
		$serv_rn = &service::factory('voa_s_cyadmin_recognition_bill');
		$serv_rn->insert(array(
			'bill_id' => $rbb_id,
			//'rb_domain' => $sets['domain'],
			'ep_id' => $sets['ep_id'],
			'rb_pictureurl' => '/attachment/read/'.$attachment['at_id']
		));
