<?php
/**
 * 	voa_c_api_reimburse_verify
 * 报销操作相关
 * $Author$
 * $Id$
 */

class voa_c_api_reimburse_verify extends voa_c_api_reimburse_base {
	/** 报销信息 */
	...
	protected function _chk_permit() {
		...
		if (empty($this->_reimburse)) {
			...
			return $this->_set_errcode(voa_errcode_api_reimburse::REIMBURSE_NOT_EXIST);
		}
		...
		if ($this->_proc['m_uid'] != startup_env::get('wbs_uid') || voa_d_oa_reimburse_proc::STATUS_NORMAL != $this->_proc['rbpc_status']) {
			...
			return $this->_set_errcode(voa_errcode_api_reimburse::REIMBURES_FORBIDDEN);
		}
