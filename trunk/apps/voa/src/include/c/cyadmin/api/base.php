<?php
/**
 * voa_c_cyadmin_api_base
 * 接口/基本控制器
 * $Author$
 * $Id$
 */
class voa_c_cyadmin_api_base extends controller {
	 ...
	 * @var string
	 */
	protected $_cookie_prefix = 'oasite_';
	...
	protected function _before_action($action) {
		...
		// 将当前企业号（域名的名）做为其cookie名前缀
		$this->_cookie_prefix = ($this->_cookie_prefix ? $this->_cookie_prefix : '').voa_h_func::get_domain().'_';
	...
	protected function _output($errcode = 0, $errmsg = '', $result = array(),$type = 'json') {
	...
	/**
	 * 判断提交的 密钥是否合法
	 * en_key 为 post 数组里的 密钥键值
	 * 经过voa.rpc.client.auth_key二次加密
	 * created by zhoutao
	 */
	protected function _check_post_password ($postx) {
		// 判断密钥是否为空
		if( empty( $postx['en_key'] ) ) {
			$this->_errcode = '10000';
			$this->_errmsg  = '操作失败,丢失密钥';

			return false;
		}
		// 判断密钥是否正确
		$key    = config::get( 'voa.rpc.client.auth_key' );
		$de_key = authcode( authcode( $postx['en_key'], $key, 'DECODE' ), $key, 'DECODE' );
		if( empty ( $de_key ) ) {
			$this->_errcode = '10001';
			$this->_errmsg  = '密钥错误,请刷新页面重试';

			return false;
		}
		// 判断是否过期
		$now_timestamp = startup_env::get( 'timestamp' );
		if( $now_timestamp - $de_key >= 300 ) {
			$this->_errcode = '10002';
			$this->_errmsg  = '密钥过期,请刷新页面重试';

			return false;
		}

		unset( $postx['en_key'] ); // 去掉密钥
		return true;
	}
}
