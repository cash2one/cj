CREATE TABLE IF NOT EXISTS `{$prefix}file{$suffix}` (
...
CREATE TABLE IF NOT EXISTS `{$prefix}file_log{$suffix}` (
...
CREATE TABLE IF NOT EXISTS `{$prefix}file_log_template{$suffix}` (
...
CREATE TABLE IF NOT EXISTS `{$prefix}file_permission{$suffix}` (
  ...
  `p_m_type` tinyint(4) NOT NULL COMMENT '成员类型: 15=组长; 12=协作者; 8=浏览者; 4=其他, 数据库存储二进制数',
...
CREATE TABLE IF NOT EXISTS `{$prefix}file_setting{$suffix}` (
...
CREATE TABLE IF NOT EXISTS `{$prefix}file_type{$suffix}` (
...
CREATE TABLE IF NOT EXISTS `{$prefix}comment{$suffix}` (
