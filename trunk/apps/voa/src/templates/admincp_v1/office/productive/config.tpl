...
<div class="panel-heading"><a href="{$subListUrl}">/根目录</a> {if $pid} > {$current['pti_name']}{/if}</div>
...
{foreach $list as $_id=>$_data}
        <tr>
            ...
            {if !$pid}
            <td>{$_data['pti_name']|escape}</td>
            {/if}
            <td>{$_data['pti_describe']|escape}</td>
            {if $pid}
            <td>{$_data['pti_rules']|escape}</td>
            {/if}
