...
<div class="panel-heading">文章列表 <button type="button" class="close"><span class="glyphicon glyphicon glyphicon-chevron-down"></span></button></div>
...
<div class="form-group" style="margin:20px 0">
	<label class="control-label col-sm-1">状态</label>
        ...
        <div class="col-sm-2">
            <select class="input-sm form-control" name="acid">
                <option value="">请选择</option> 
                <option value="1" {if $conds['acid'] == 1}selected{/if}>媒体报道</option>
				<option value="2" {if $conds['acid'] == 2}selected{/if}>热门活动</option>
				<option value="3" {if $conds['acid'] == 3}selected{/if}>在线听课</option>
				<option value="4" {if $conds['acid'] == 4}selected{/if}>企业微信</option>
              </select>
            <span class="help-block"></span>
        </div>
...
{/if}
	<tbody>
{if $article_list}
	{foreach $article_list as $_id => $_data}
		<tr>
			<td class="text-left"><label class="px-single"><input type="checkbox" class="px" name="delete[{$_id}]" value="{$_data['aid']}"{if !$form_delete_url} disabled="disabled"{/if} /><span class="lbl"> </span></label></td>
			<td><a href="/content/article/view/?aid={$_data['aid']}">{$_data['title']|escape}</a></td>
			<td align="center">{$_data['cname']}</td>
			<td align="center">{$_data['status']}</td>
			<td align="center">{$_data['asort']}</td>
			
			<td align="center">{$_data['time']|escape}</td>
			<td align="center">
				 {$base->show_link($edit_url, $_data['aid'], '编辑', 'fa-edit')}
				 {$base->show_link($delete_url, $_data['aid'], '删除', 'fa-delete')}
			</td>
		</tr>
	{/foreach}
{else}
