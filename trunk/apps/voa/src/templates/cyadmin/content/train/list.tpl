...
<div id="content-wrapper">
	<div class="panel panel-default">
		...
		<div class="panel-heading">线下培训 <button type="button" class="close"><span class="glyphicon glyphicon glyphicon-chevron-down"></span></button></div>
		<div class="panel-body">
			...
			<form class="form-horizontal" role="form" method="post" action="{$form_delete_url}?delete">
				...
				<tbody>
					{if $train_list}
					{foreach $train_list as $_id => $_data}
					<tr>
						<td class="text-left"><label class="px-single"><input type="checkbox" class="px" name="delete[{$_id}]" value="{$_data['tid']}"{if !$form_delete_url} disabled="disabled"{/if} /><span class="lbl"> </span></label></td>
						<td><a href="/content/train/view/?tid={$_data['tid']}">{$_data['title']|escape}</a></td>
						<td align="center">{$_data['status']}</td>
						<td align="center">{$_data['tsort']}</td>

						<td align="center">{$_data['time']|escape}</td>
						<td align="center">
							{$base->show_link($edit_url, $_data['tid'], '编辑', 'fa-edit')}
							{$base->show_link($delete_url, $_data['tid'], '删除', 'fa-delete')}
						</td>
					</tr>
					{/foreach}
					{else}
