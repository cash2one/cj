...
<div class="panel-heading">招聘列表 <button type="button" class="close"><span class="glyphicon glyphicon glyphicon-chevron-down"></span></button></div>
...
{/if}
	<tbody>
{if $join_list}
	{foreach $join_list as $_id => $_data}
		<tr>
			<td class="text-left"><label class="px-single"><input type="checkbox" class="px" name="delete[{$_id}]" value="{$_data['jid']}"{if !$form_delete_url} disabled="disabled"{/if} /><span class="lbl"> </span></label></td>
			<td><a href="/content/join/view/?jid={$_data['jid']}">{$_data['jobname']|escape}</a></td>
			<td align="center">{$_data['status']}</td>
			<td align="center">{$_data['jsort']}</td>
			
			<td align="center">{$_data['time']|escape}</td>
			<td align="center">
				 {$base->show_link($edit_url, $_data['jid'], '编辑', 'fa-edit')}
				 {$base->show_link($delete_url, $_data['jid'], '删除', 'fa-delete')}
			</td>
		</tr>
	{/foreach}
{else}
		<tr>
			<td colspan="6" class="warning">{if $issearch}未搜索到指定条件的数据{else}暂无任何数据{/if}</td>
		</tr>
{/if}
	</tbody>
</table>
