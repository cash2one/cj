...
<nav class="mod_common_list_style">
	<div class="day" data-range="30" data-init="{$ymd}"><select></select></div>
	<div class="city" data-init="{$parent_shop['cr_name']}"><select>
		<option selected value="-1">全部{$productive_set['title_city']}</option>
	</select></div>
	<div class="dist" data-init="{$cur_shop['cr_name']}"><select>
		<option value="-1">全部{$productive_set['title_region']}</option>
	</select></div>
</nav>

{if empty($list)}
<em class="mod_empty_notice mod_common_list_style"><span>没有您需要查看的反馈记录</span></em>
{else}
<ul class="mod_common_list shoplist">
	{include file='frontend/productive/rank_li.tpl'}
</ul>
{/if}
...
require(['dialog', 'business'], function() {
	$onload(function() {
		...
		_more.init('show_more', 'productive_list', {'page':'_page'});
	});

	function checkSubmit(e){
		var $daySel = $one('.day select');
		var $citySel = $one('.city select');
		var $distSel = $one('.dist select');
		
		if ($distSel.value != -1){
			var url = '/frontend/productive/rank/ymd/' + $daySel.value + '/pid/' + $citySel.value + '/cid/' + $distSel.value;
			location.href = url;
		}
	}
	...
	(function(){
		...
		while(range--){
			...
			selected = $data(sel.parentNode, 'init') == flag ? ' selected ' : '';
			$prepend(sel, '<option value="'+ flag +'" '+ selected + '>'+fmtDate(day, '/')+'&nbsp;</option>');
		}
		if (selected === null){
			sel.selectedIndex = sel.options.length - 1;
		}
		MOA.event.listenSelectChange(sel, checkSubmit);
	}());
	function fixNum(n){
