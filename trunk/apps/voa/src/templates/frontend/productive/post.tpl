...
<div id="viewstack"><section>
	...
	<input type="hidden" name="ptd_id" value="{$ptd_id}" />
		...
		<ul class="totalScore mod_common_list">
			<li>
				<a href="/frontend/productive/editem/pt_id/{$productive['pt_id']}" class="m_link">
					<label>{if 0 < $productive_set['score_rule_diy']}合格率{else}各项总分{/if}</label>
					<em>{$total}{if 0 < $productive_set['score_rule_diy']}%{else}分{/if}</em>
					<span>查看评估指标</span>
				</a>
			</li>
		</ul>
