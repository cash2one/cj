...
<header class="single">
	<img src="{$cinstance->avatar($userinfo['m_uid'])}" />
	<h1>{$userinfo['m_username']}</h1>
	<h2>{if !empty($jobs[$userinfo['cj_id']])}{$jobs[$userinfo.cj_id]['cj_name']}{/if}</h2>
	<div>
		<time>1<i>月</i>1<i>日</i></time>
		<select name="" data-from="{$date_from}" data-to="{$date_to}" data-init="{$date_current}"></select>
		<h3>销售轨迹</h3>
		<a class="table" href="/footprint/report/{$userinfo['m_uid']}">报表</a>
	</div>
</header>
...
require(['dialog', 'members', 'business'], function(){
	$onload(function(){
		...
		parseHiddenSelect(sel, function($sel, $fake, value){
			updateFake(value);
			window.location.href = '/footprint/list/' + _cur_uid + '?btime=' + value;
		});
	});
});
{/literal}
