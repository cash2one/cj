...
<div id="{$iptname}_div" class="mod_members to">
	<ul class="box">
		{if $accepter}
		<li data-id="{$accepter['m_uid']}" id="{$accepter['m_uid']}" class="newjoin"><a class="rm" href="javascript:void(0)">删除</a><img src="{$cinstance->avatar($accepter['m_uid'])}" />{$accepter['m_username']}</li>
		{/if}
