...
<form class="form-horizontal font12" id="submit-adminergroup" role="form" method="POST" action="{$editTargetUrl}">
...
{/if}
	...
	<div class="form-group">
		<div class="col-sm-offset-2 col-sm-6">
			<button type="submit" class="btn btn-primary">{if $groupDetail['cag_id']}保存{else}添加{/if}</button>
			&nbsp;&nbsp;
...
jQuery(function () {
	jQuery('#submit-adminergroup').submit(function () {

		// 存在权限设置，则检查是否设置了
		if (jQuery('input[name^="cag_role"]').length > 0) {
			var role_is_set = false;
			jQuery.each(jQuery('input[name^="cag_role"]'), function (i, o) {
				if (jQuery(o).prop('checked')) {
					role_is_set = true;
					return false;
				}
			});
