 ...
 * 追加一行数据
 */
function _set_row(id, data, msg, success, ignore) {
	jQuery('#import-list').append(txTpl('tpl-import-list', {
		"id": id,
		"region_level_1": data.region_level_1,
		"region_level_2": data.region_level_2,
		"region_level_3": data.region_level_3,
		"name": data.name,
...
function _batch_import(result) {
	...
	jQuery.each(result.data_list, function(i, data) {
		...
		jQuery.ajax({
			...
			"success": function(r) {
				if (typeof(r.errcode) === 'undefined') {
					_set_row(i, data, '未知解析错误', false);
					return false;
				}
				if (r.errcode > 0) {
					_set_row(i, data, r.errmsg, false, r.errcode == 1007 ? true : false);
					return false;
				}
				// 导入成功
...
<% if (success) { %>
	<td>-</td>
	<td><%=region_level_1%></td>
	<td><%=region_level_2%></td>
	<td><%=region_level_3%></td>
	<td><%=name%></td>
	<td><%=address%></td>
	<td><%=master%></td>
	<td><%=normal%></td>
...
<% } else { %>
	...
	<td><input type="text" name="region_level_1[<%=id%>]" class="form-control" placeholder="门店区域" required="required" value="<%=region_level_1%>" /></td>
	<td><input type="text" name="region_level_2[<%=id%>]" class="form-control" placeholder="门店区域" value="<%=region_level_2%>" /></td>
	<td><input type="text" name="region_level_3[<%=id%>]" class="form-control" placeholder="门店区域" value="<%=region_level_3%>" /></td>
	<td><input type="text" name="name[<%=id%>]" class="form-control" placeholder="门店名称" required="required" value="<%=name%>" /></td>
	<td><input type="text" name="address[<%=id%>]" class="form-control" placeholder="门店地址" required="required" value="<%=address%>" /></td>
	<td><input type="text" name="master[<%=id%>]" class="form-control" placeholder="门店负责人" required="required" value="<%=master%>" /></td>
	<td><input type="text" name="normal[<%=id%>]" class="form-control" placeholder="门店成员" value="<%=normal%>" /></td>
	<td><%=msg%></td>
<% } %>
</tr>
</script>
