...
<div class="panel panel-default font12">
	<div class="panel-body">
		...
		<form class="form-horizontal font12" role="form" action="{$form_action_url}" method="post" data-ng-app="ng.poler.plugins.pc">
			...
			<input type="hidden" name="formhash" value="{$formhash}" />
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<select id="id_tc_id" name="tc_id" class="form-control form-small" data-width="auto"  required="required">						
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<input type="text" class="form-control form-small" id="id_title" name="title" placeholder="最多输入17个汉字" value="{$article['title']|escape}" maxlength="17"  required="required"/>			
					</div>
				</div>
				<div class="form-group">
					<label class="control-label  col-sm-2 text-danger" for="id_author">作者*</label>
					<div class="col-sm-9">
						<input type="text" class="form-control form-small" id="id_author" name="author" placeholder="最多输入15个汉字" value="{$article['author']|escape}" maxlength="15"  required="required"/>			
					</div>
				</div>
				<div class="form-group">
		...
		{else}
			尚未创建文章目录，请先<a  href='{$addCategoryUrl}'>添加目录</a>	
		{/if}
	</div>
</div>
...
{*jQuery(function(){*}
	...
	{*$('#id_tc_id').on('change', function(){*}
		...
		{**}
		{*$.ajax({*}
			{*"url": "/api/train/get/categoryright",*}
			{*"dataType": "json",*}
			{*"type": "POST",*}
			{*"data": {*}
				{*"tc_id": tc_id*}
