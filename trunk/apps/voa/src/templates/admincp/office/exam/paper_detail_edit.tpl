...
<table class="table table-striped table-hover table-bordered font12">
	...
	<thead>
		<tr>
			...
			<th>题目名称</th>
			<th>题型</th>
			<th><label class="checkbox"><input type="checkbox" class="px" onchange="javascript:checkAll(this,'detail_ids');" /><span class="lbl">选择</span></label></th>
			<th>分数</th>
			<th>排序</th>
		</tr>
	</thead>
	...
	{foreach $tis as $_id => $_data}
		<tr>
			...
			<td><input type="checkbox" name="detail_ids[]" value="{$_data['detail_id']}" checked /></td>
			<td>
				{$_data['score']}
			</td>
			<td><input type="text" name="orders[{$_data['detail_id']}]"  value="{$_data['orderby']}"/></td>
		</tr>
	{/foreach}
	</tbody>
</table>
	<div class="form-group" id="btn-box">
		<div class="col-sm-offset-2 col-sm-9">
			<div class="row">
				...
				<div class="col-md-3">
					<input type="submit" name="resetsubmit" value="重新抽题" class="btn btn-primary  col-md-9">
				</div>
				<div class="col-md-3">
					<input type="submit" name="editsubmit" value="下一步" class="btn btn-primary  col-md-9" id="draft_btn">
				</div>
			</div>
		</div>
	</div>
...
$('#draft_btn').bind('click', function () {
	var ids = [];
	$('input[name="detail_ids[]"]:checked').each(function(){
		//ids+=$(this).val();
		ids.push($(this).val());
	});
	if(ids.length==0){
