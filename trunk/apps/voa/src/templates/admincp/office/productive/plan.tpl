...
<div class="panel-heading">列表 <button type="button" class="close"><span class="glyphicon glyphicon glyphicon-chevron-down"></span></button></div>
  ...
  <div class="form-group ">
       ...
       <div class="col-md-4" id="sandbox-container">
            ...
            <input type="text" class="input-sm form-control" value="{$search['ptt_start_date']}" name="search[ptt_start_date]">
            <span class="input-group-addon">到</span>
            <input type="text" class="input-sm form-control" value="{$search['ptt_end_date']}" name="search[ptt_end_date]">
            </div>
        </div>
    ...
    <div class="form-group " style="margin-bottom:0;">  
        ...
        <div class="col-sm-2">
            <input type="hidden" class="users" value="{$search['ptt_assign_users']}" >
            <input type="hidden" name="search[ptt_assign_uid]" class="it-uid" value="{$search['ptt_assign_uid']}" >
        ...
        <div class="col-sm-2">
            <input type="hidden" class="users" value="{$search['ptt_submit_users']}" >
            <input type="hidden" name="search[ptt_submit_uid]" class="it-uid" value="{$search['ptt_submit_uid']}" >
            
            <input type="text" value="{$search['ptt_submit_uid']}" placeholder="" class="input-sm form-control token-m-username">
...
{foreach $list as $_id=>$_data}
        <tr>
            ...
            <!--<td>{$_data['ptt_title']|escape}</td>-->
            <td>{$_data['ptt_start_date']|escape}</td>
            <td>{$_data['ptt_submit_username']}</td>
            <td>{$_data['ptt_assign_users']}</td>
            <td>{$_data['ptt_csp_id_list_total']}</td>
            <td>{$_data['ptt_finished_total']}</td>
            <td>{$_data['ptt_execution_status_text']}</td>

            <td>
                {if $_data['ptt_execution_status'] == 1 || $_data['ptt_execution_status'] == 3} 
                    {$base->linkShow($deleteUrlBase, $_id, '删除', 'fa-times', 'class="_delete"')} | 
                    {$base->linkShow($editUrl, $_id, '编辑', 'fa-edit', '')}
                    {if $_data['ptt_csp_id_list_total'] && $_data['ptt_assign_users']}
