		...
		<div class="profile-row">
				...
				<div class="profile-block">
					<div class="panel profile-photo">
						<div class="padding-sm">
							<span class="panel-title" id="fill_title">{$news['title']|escape}</span>
						</div>
						<div class="img-box" id="fill_cover">
							<img src="{$news['cover']['url']}" alt="" id="fill_cover">
						</div>
					</div>
				</div>
			</div>
			<div class="right-col">
				<div class="panel tl-body">
					<form class="form-horizontal font12" role="form" action="{$form_action_url}" method="post" id="post_from" data-ng-app="ng.poler.plugins.pc">
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								<input type="text" class="form-control form-small" id="id_title" name="title" placeholder="最多输入64个字符" value="{$news['title']|escape}" maxlength="64"  required="required"/>
							</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								<textarea  class="form-control form-small" id="id_summary" name="summary" placeholder="最多输入120个字符" maxlength="120" rows=4>{$news['summary']|escape}</textarea>
							</div>
						</div>
						<div class="form-group">
										...
										<optgroup label="{$_val['name']}">
											{if isset($_val['nodes'])}
												{foreach $_val['nodes'] as $_sv}
													<option value="{$_sv['nca_id']}" {if $news['nca_id'] == $_sv['nca_id']} selected="selected"{/if}>{$_sv['name']}</option>
												{/foreach}
											{else}
												<option value="{$_val['nca_id']}" {if $news['nca_id'] == $_val['nca_id']} selected="selected"{/if}>{$_val['name']}</option>
											{/if}
										</optgroup>
									{/foreach}
								</select>
						...
						<div class="form-group">
							...
							<div class="col-sm-1">
								<input type="radio" class=" form-small" id="is_secret" name="is_secret" value="0" {if $news['is_secret'] == 0}checked{/if} />&nbsp;&nbsp;关闭
							</div>
							<div class="col-sm-6">
								<input type="radio" class=" form-small" id="is_secret" name="is_secret" value="1" {if $news['is_secret'] == 1}checked{/if}/>&nbsp;&nbsp;开启
							</div>
							<div class="col-sm-2">
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								<div>
									<input type="hidden" class="form-small" id="is_all" name="is_all" value="{$news['is_all']}"/>
									<button type="button" class="btn {if $news['is_all'] == 1}btn-primary{/if}" id="all_btn">全公司</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<button type="button" class="btn {if $news['is_all'] == 0}btn-primary{/if}" id="specified_btn">指定对象</button>
								...
								<div id="user_dep_container" {if $news['is_all'] == 1}style="display: none" {/if}>
									<hr>
									<div class="row">
						...
						<div class="form-group">
							<div class="col-sm-3 col-sm-offset-2">
								<input type="checkbox" class=" form-small"  name="is_comment" value="1"{if $news['is_comment'] == 1}checked{/if}/>&nbsp;&nbsp;开启评论功能
							</div>
							<div class="col-sm-3 col-sm-offset-2">
								<input type="checkbox" class="form-small"  name="is_like" value="1" {if $news['is_like'] == 1}checked{/if}/>&nbsp;&nbsp;开启点赞功能
							</div>
						</div>
						{if $news['is_publish'] != 1}
							...
							<div class="check">
								<div class="from-group">
									<label class="control-label col-sm-2 " for="check_summary">审核说明</label>
									<div class="col-sm-9">
								...
								<div class="from-group">
									<label class="control-label col-sm-2 " for="users_check">审核人</label>
									<div class="col-sm-9">
								...
								<div class="form-group">
									<div class="col-sm-offset-2 col-sm-9">
										<div class="row">
											<div class="col-md-4">
												<button type="submit" class="btn btn-primary col-md-9"  data-toggle="modal" id="preview_btn">提交审核</button>
											</div>
										</div>
									</div>
								</div>
...
<script type="text/javascript">
	...
	if (author.length != 0) {
		...
		for (var i = 0; i < author.length; i ++) {
			author_choose += '<input name="author" value="' + author[i]['m_uid'] + '" type="hidden">';
			select_m_uid_name += author[i]['m_username'] + ' ';
		}
	...
	auditor = {$default_check};
	if (auditor.length != 0) {
		auditor_choose = '';
		var select_auditor_name = '';
		for (var i = 0; i < auditor.length; i ++) {
			auditor_choose += '<input name="check_id[]" value="' + auditor[i]['m_uid'] + '" type="hidden">';
			select_auditor_name += auditor[i]['m_username'] + ' ';
		}
