    ...
    $('#fileupload').fileupload({
        ...
        progressall: function (e, data) {
            ...
            if (data.result.status == 100) {
                ...
                alert(data.result.result.error);
            }
  ...
  <div class="panel-body">
        ...
        <div class="col-sm-5">
            <select name="search[city][]" title="选择{$cache_config['title_city']}" data-header="可以选择多个{$cache_config['title_city']}" class="selectpicker  selectpicker-city col-lg-5" data-live-search="true" multiple>
                    ...
                    {if $_data['cr_parent_id'] eq 0}<option value="{$_data['cr_id']}" {if $search['city'] && in_array($_data['cr_id'], $search['city'])}selected{/if}>{$_data['cr_name']}</option>{/if}
                    {/foreach}
                    </select>
        ...
        <button name="submit" value="1" type="submit" class="btn btn-info input-sm"><i class="fa fa-search"></i> 搜索</button> <span class="space"></span>  
        <span id="btn-add" class="btn btn-primary">
        ...
        <div class="table-caption font12">
            ...
            <div class="DT-lf-right">
                <form  id="fileupload" method="POST"  enctype="multipart/form-data">
                     <span id="btn-download" class="btn btn-default btn-sm">
...
<table class="table table-striped table-bordered table-hover font12">
    ...
    <thead>
        <tr>
            <th>
                <label class="checkbox"><input type="checkbox" id="delete-all" class="px" onchange="javascript:checkAll(this,'delete');"{if !$deleteUrlBase} disabled="disabled"{/if} /><span class="lbl">全选</span></label>
            </th>
...
{if $total > 0}
    <tfoot>
        <tr>
            <td colspan="2">{if $deleteUrlBase}<button type="submit" class="btn btn btn-danger">批量删除</button>{/if}</td>
...
{foreach $list as $_id=>$_data}
        <tr>
            <td class="text-left"><label class="px-single"><input type="checkbox" class="px" name="delete[{$_id}]" value="{$_id}"{if !$deleteUrlBase} disabled="disabled"{/if} /><span class="lbl"> </span></label></td>
