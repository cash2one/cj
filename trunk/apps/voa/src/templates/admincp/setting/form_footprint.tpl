			...
			<div class="form-group font12">
				...
				<div class="col-sm-9">
					...
					<table id="types-cp" class="table table-condensed">
						...
						<tbody>
			{$max_key = 0}
			{foreach $fs['value']['type'] as $type_key => $type_name}
							<tr>
								<td><label class="checkbox-inline"><input type="checkbox" name="_types_delete[{$type_key}]" value="{$type_key}"{if in_array($type_key, $fs['value']['system'])} disabled="disabled"{/if} /><span class="space"></span></label></td>
