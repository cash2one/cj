{foreach $formCustomFields as $fs}
	{if $fs['id'] == 'types'}
			...
			{foreach $fs['value'] as $type_key => $type_name}
							<tr>
								<td><label class="checkbox-inline"><input type="checkbox" class="px" name="_types_delete[{$type_key}]" value="{$type_key}" /><span class="space"></span></label></td>
			...
			{$max_key = $max_key + 1}
						...
						<tfoot>
							<tr>
								<td><a href="javascript:;" onclick="javascript:_add_types();" role="button" class="btn btn-primary">新增</a></td>
