...
(function (app) {
     ...
     * 页面列表缓存
     */
    app.factory('ListCacheFactory', ['$location', '$q', '$timeout', 'Page', '$document', '$anchorScroll',
        function ($location, $q, $timeout, Page, $document, $anchorScroll) {
    ...
    /**
     * fixme：ListPageFactory 这个名字很不好，它只是 ListCacheFactory 的一个别名，
     *        但是由于之前项目中使用这个名字，所以我不得不在提供它
     *        建议不要使用 ListPageFactory，直接使用 ListCacheFactory
     */
        .factory('ListPageFactory', ['ListCacheFactory', function (ListCacheFactory) {
            return ListCacheFactory;
        }])
    ;
})(angular.module('ng.poler'));
/**
