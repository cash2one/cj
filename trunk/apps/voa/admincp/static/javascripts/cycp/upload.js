	...
	jQuery('body').on('click', '.fileinput-button', function () {
		jQuery('.cycp_uploader').fileupload({
			...
			"done": function (e, data) {
				...
				// 显示图片
				if ($t.attr('data-showimage')) {
					var html = '<a href="' + result['list'][0]['url'] + '" target="_blank">';
					html += '<img src="' + result['list'][0]['thumbnailUrl'] + '" border="0" style="max-width:64px;max-height:32px" />';
					html += '</a>';
					$box.find('._showimage').html(html);
				}
