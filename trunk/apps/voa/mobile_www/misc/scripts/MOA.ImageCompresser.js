...
!function(a) {
    !function(b) {
        function c(a) {
            ...
            this.encode = function(a, b) {
                ...
                for (var h, j, q, s, x, y, z, A, D, E = a.data, F = a.width, G = a.height, N = 4 * F, P = 0; G > P; ) {
                    for (h = 0; N > h; ) {
                        for (x = N * P + h, y = x, z = -1, A = 0, D = 0; 64 > D; D++) A = D >> 3, z = 4 * (7 & D), 
                        y = x + A * N + z, P + A >= G && (y -= N * (P + 1 + A - G)), h + z >= N && (y -= h + z - N + 4), 
                        j = E[y++], q = E[y++], s = E[y++], K[D] = (O[j] + O[q + 256 >> 0] + O[s + 512 >> 0] >> 16) - 128, 
                        L[D] = (O[j + 768 >> 0] + O[q + 1024 >> 0] + O[s + 1280 >> 0] >> 16) - 128, M[D] = (O[j + 1280 >> 0] + O[q + 1536 >> 0] + O[s + 1792 >> 0] >> 16) - 128;
        ...
        b.ImageCompresser = {
            isIosSubSample: function(a) {
                var b = a.naturalWidth, c = a.naturalHeight, d = !1;
                if (b * c > 1048576) {
                    var e = document.createElement("canvas"), f = e.getContext("2d");
                    e.width = 1, e.height = 1, f.drawImage(a, 1 - b, 0), d = 0 === f.getImageData(0, 0, 1, 1).data[3], 
                    e = f = null;
                }
                return d;
            },
            getIosImageRatio: function(a, b, c) {
                var d, e = document.createElement("canvas"), f = e.getContext("2d"), g = 0, h = c, i = c;
                for (e.width = 1, e.height = c, f.drawImage(a, 1 - Math.ceil(Math.random() * b), 0), 
            ...
            getImageBase64: function(d, e) {
                ...
                var h, i, j = f.maxW, k = f.maxW, l = f.quality, m = d.naturalWidth, n = d.naturalHeight;
                a.env.ios && b.ImageCompresser.isIosSubSample(d) && (m /= 2, n /= 2), m > j && m / n >= j / k ? (h = j, 
                i = n * j / m) : n > k && n / m >= k / j ? (h = m * k / n, i = k) : (h = m, i = n);
                var o, p = document.createElement("canvas"), q = p.getContext("2d");
                if (this.doAutoRotate(p, h, i, f.orien), a.env.ios) {
                    var r, s, t, u, g, v, w, x, y = document.createElement("canvas"), z = y.getContext("2d"), A = 1024, B = b.ImageCompresser.getIosImageRatio(d, m, n);
                    for (y.width = y.height = A, s = 0; n > s; ) {
                        for (u = s + A > n ? n - s : A, r = 0; m > r; ) t = r + A > m ? m - r : A, z.clearRect(0, 0, A, A), 
                        z.drawImage(d, -r, -s), g = Math.floor(r * h / m), w = Math.ceil(t * h / m), v = Math.floor(s * i / n / B), 
                        x = Math.ceil(u * i / n / B), q.drawImage(y, 0, 0, t, u, g, v, w, x), r += A;
                        s += A;
                    }
                    y = z = null;
                } else q.drawImage(d, 0, 0, m, n, 0, 0, h, i);
                if (a.env.android) {
                    var C = q.getImageData(0, 0, p.width, p.height), D = new c(100 * l);
                    o = D.encode(C), D = null;
                } else o = p.toDataURL("image/jpeg", l);
                return p = q = null, o;
            },
            doAutoRotate: function(a, b, c, d) {
                var e = a.getContext("2d");
                switch (d >= 5 && 8 >= d ? (a.width = c, a.height = b) : (a.width = b, a.height = c), 
                d) {
                  case 2:
            ...
            getFileObjectURL: function(a) {
                var b = window.URL || window.webkitURL || !1;
                return b ? b.createObjectURL(a) : void 0;
            },
            support: function() {
