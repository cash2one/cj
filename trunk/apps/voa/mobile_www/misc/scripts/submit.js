	...
	Submit.prototype = {
		...
		init: function(sets, ajax) {
			...
			this._form.on("submit", function(e) {
				...
				self._ajax["url"] = self._createurl(action, {"inajax": 1});
		...
		_ajax_success: function(data, status, xhr) {
			...
			if (null != this._cb_success) {
				return this._cb_success(data, status, xhr);
			}
			...
			if (_.has(result, "url") && _.isString(result["url"]) && 0 < result["url"].length) {
				// 如果有提示, 则
				...
				var __self = this;
				setTimeout(function() {
					__self._loading = true;
					window.location = result["url"];
				}, 500);
			} else {
				this.show_error(result["message"] + (0 < data["errcode"] ? "(code:" + data["errcode"] + ")" : ""), null, null);
			}
