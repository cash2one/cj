/**
 * Created by mojianyuan on 12/26/14.
 */
/*!
...
$.widgets.Carousel = function(wargs) {
    ...
    this.addItem = function(item) {
        ...
        $("<div id='carouse_item_" + item.id+"'>"+text+"</div>").css("zIndex", 1).width(itemWidth).height(itemHeight).css('display', 'inline').css("float", "left").appendTo(_widget.container)
        $("#carouse_item_" + id).height(itemHeight - 50);
        item.div = $("<div id='carouse_item_" + item.id+"'>"+text+"</div>");
        items.push(item);
    ...
    this.init = function() {
        ...
        _widget.container = wargs.uuid.find("#carousel_content");
        _widget.scrollpane = wargs.uuid.find("#carousel_scrollpane");
        ...
        _widget.nav = wargs.uuid.find("#carousel_nav");
        _widget.mid = wargs.uuid.find("#carousel_mid");
        _widget.node.addClass("carousel-theme");
        template = unescape(template.html() + "");
